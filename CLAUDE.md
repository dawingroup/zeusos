...
- **Tools registry partially cleaned**: `designTools.js` + `manufacturingTools.js` + `inventoryTools.js` (the three pure-DawinOS tool modules behind the `crossModuleIntelligence` AI assistant) were stripped from `functions/src/tools/index.js` and deleted. Same PR removed `mergeInventoryDuplicates` (DawinOS inventory AI) and the `generateDesignManagerEvents{,HTTP}` + `processPendingEvents` admin triggers from `functions/index.js` since they had no frontend callers. **Still needs a follow-up sweep**: `crmTools.js` / `financeTools.js` / `crossModuleTools.js` have *mixed* usage — they reference active ZeusOS collections (customers, crm_deals, suppliers, purchase_orders) alongside dead ones (designProjects, manufacturingOrders). Cleaning those requires re-pointing the dead-collection branches at the ZeusOS commercial-gravity equivalents (master_jobs / quotes / client_invoices) with proper domain analysis. Once that lands, `firestore.rules` can drop the `bom`, `materials`, `inventoryItems`, `finishLibrary`, `designProjects`, `designItems` match blocks. Also live readers still pinning those rules: `functions/src/admin/backfillMaterialFields.js`.
