  ...
  return (
    <div className="relative">
      ...
      {showSuggestions && !query && (
        <div className="absolute top-full mt-2 w-full bg-card rounded-xl border border-[var(--border-subtle)] shadow-lg z-50">
          <div className="p-3">
            ...
            <div className="space-y-1">
              {suggestions.map((suggestion, i) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-sm text-muted-foreground rounded-lg hover:bg-[var(--bg-sunken)] transition-colors"
                >
                  {suggestion}
                </button>
              ))}
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {history.slice(0, 5).map((item, i) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-lg hover:bg-[var(--bg-sunken)] transition-colors"
                >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1 bg-[var(--bg-sunken)] rounded-lg p-1">
          <button
            ...
            className={`p-1.5 rounded ${viewMode === 'table' ? 'bg-card shadow-sm' : 'text-muted-foreground'}`}
          >
            <Table className="w-4 h-4" />
          </button>
          <button
            onClick={() => setViewMode('cards')}
            className={`p-1.5 rounded ${viewMode === 'cards' ? 'bg-card shadow-sm' : 'text-muted-foreground'}`}
          >
            <LayoutGrid className="w-4 h-4" />
          </button>
          <button
            onClick={() => setViewMode('chart')}
            className={`p-1.5 rounded ${viewMode === 'chart' ? 'bg-card shadow-sm' : 'text-muted-foreground'}`}
          >
      ...
      {response.results.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {response.results.slice(0, 20).map((row, i) => (
                <tr key={row.id || i} className="border-b border-[var(--border-subtle)] hover:bg-[var(--bg-sunken)]">
                  {Object.entries(row)
                    .filter(([key]) => key !== 'id')
                    .slice(0, 5)
                    .map(([key, value]) => (
                      <td key={key} className="py-2 px-3 text-muted-foreground">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {response.results.slice(0, 12).map((item, i) => (
            <div key={item.id || i} className="p-4 bg-[var(--bg-sunken)] rounded-lg border border-[var(--border-subtle)]">
        ...
        <div className="h-64 flex items-center justify-center bg-[var(--bg-sunken)] rounded-lg border border-[var(--border-subtle)]">
