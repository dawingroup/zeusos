...
export async function cleanupBOQItems(
  ...
  for (const item of items) {
    ...
    // Use upstream hierarchyLevel as a hint if available (from extractBOQItem)
    const upstreamLevel = (item as any).hierarchyLevel as number | undefined;
    ...
    // If no item code, use tracked context or upstream hint
    if (codeParts.length === 0) {
      billNumber = currentBillNumber;
      elementCode = currentElementCode;
      sectionCode = currentSectionCode;
      if (upstreamLevel && upstreamLevel > 0) {
        // Trust upstream hierarchy detection
        hierarchyLevel = upstreamLevel;
      } else {
    ...
    // Use AI-enhanced fields if available (from Claude Cloud Function)
    const aiItem = item as any;
    const aiCategory = aiItem.aiEnhanced ? aiItem.category : undefined;
    const aiStage = aiItem.aiEnhanced ? aiItem.stage : undefined;

    const cleanedItem: CleanedBOQItem = {
      ...item,
      itemName: aiItem.aiDescription || name,
      specifications: aiItem.aiSpecs || specifications,
      ...
      category: aiCategory || item.category || detectedCategory,
      stage: aiStage || item.stage || detectedStage,
      cleanupNotes: [],
    };
