  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-foreground">Growth Analysis</h2>
                ...
                <tbody>
                  {topAccounts.map((row, i) => {
                    ...
                    return (
                      ...
                      >
                        <td className="py-2 px-4 text-muted-foreground font-medium truncate max-w-[200px]">
                          {row.label}
                        </td>
                        <td className="py-2 px-4">
                          <span
                            className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${
                              CLASS_COLORS[row.classification] ?? 'bg-[var(--bg-sunken)] text-muted-foreground'
                            }`}
                          >
                            {row.classification}
                          </span>
                        ...
                        <td className="py-2 px-4 text-right">
                          {row.momPct !== null ? (
                            <span className={row.momPct >= 0 ? 'text-[var(--rag-green)]' : 'text-[var(--rag-red)]'}>
                              {row.momPct >= 0 ? '+' : ''}
                              {fmtPct(row.momPct)}
                            </span>
                          ) : (
                            <span className="text-[var(--fg-tertiary)]">N/A</span>
                          )}
                        </td>
                      </tr>
                    );
                  ...
                  {topAccounts.length === 0 && (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-[var(--fg-tertiary)]">
