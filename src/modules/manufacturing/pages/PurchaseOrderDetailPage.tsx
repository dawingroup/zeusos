...
import type { POLineItem, PurchaseOrderStatus, LandedCostDistributionMethod, POLineItemCategory } from '../types/purchaseOrder';
...
import { POItemPicker } from '../components/po/POItemPicker';
import { SupplierPicker } from '../components/po/SupplierPicker';
...
export default function PurchaseOrderDetailPage() {
  ...
  const saveAllChanges = async () => {
    ...
    await wrap(async () => {
      await actions.update({
        ...
        notes: editNotes,
        orderDate: new Date(editOrderDate),
      });
      setEditing(false);
    });
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate('/manufacturing/purchase-orders');
    } catch {
          ...
          <div>
            ...
            {editing ? (
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {order.orderDate && ` | Order Date: ${new Date((order.orderDate as any).seconds * 1000).toLocaleDateString()}`}
              </p>
            )}
          </div>
        </div>
                        ...
                        <tr key={li.id} className={isLocked ? 'bg-amber-50/40' : ''}>
                          ...
                          <td className="px-4 py-2.5">
                            <Input
                              ...
                              className="h-8"
                              disabled={isLocked || (li.category ?? 'inventory') !== 'inventory'}
                        ...
                        <tr key={li.id} className="hover:bg-muted/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-sm">
                            <Badge variant="outline" className="text-xs">
                              {PO_LINE_CATEGORY_LABELS[li.category ?? 'inventory']}
                            </Badge>
                          </td>
            ...
            <Card>
              ...
              <CardContent>
                <QBOSyncStatusBadge
                  ...
                  lastSyncedAt={order.qboSyncedAt}
