...
export function EditGoodsReceiptDialog({ open, onClose, order, receipt, onSave }: Props) {
  ...
  const [receivedDate, setReceivedDate] = useState(() => {
    if (receipt.receivedDate && typeof (receipt.receivedDate as any).toDate === 'function') {
      return (receipt.receivedDate as any).toDate().toISOString().split('T')[0];
    }
  ...
  const handleSubmit = async () => {
    ...
    const originalDate = receipt.receivedDate && typeof (receipt.receivedDate as any).toDate === 'function'
      ? (receipt.receivedDate as any).toDate().toISOString().split('T')[0]
      : null;
