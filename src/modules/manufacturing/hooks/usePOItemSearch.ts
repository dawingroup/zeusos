    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        const unified: UnifiedSearchResult[] = [
          ...inventoryItems.map((item) => ({ source: 'inventory' as const, item })),
          ...materialsWithLinks,
 ...
 */
export function resolveInventoryItem(item: InventoryListItem): ResolvedLineItemData {
  return {
    inventoryItemId: item.id,
    description: item.displayName || item.name,
    sku: item.sku,
    unitCost: item.costPerUnit ?? 0,
    unit: item.category === 'sheet-goods' ? 'sheet' : 'pcs',
...
export function resolveLinkedMaterial(
  ...
  return {
    ...
    sku: inventory.sku,
    unitCost: inventory.pricing?.costPerUnit ?? material.pricing?.unitCost ?? 0,
    unit: inventory.pricing?.unit ?? material.pricing?.unit ?? 'pcs',
