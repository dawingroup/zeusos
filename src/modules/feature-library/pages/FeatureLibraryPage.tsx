...
import { Search, Download, Database, Layers, CheckCircle, Clock, Plus } from 'lucide-react';
...
export function FeatureLibraryPage() {
  ...
  const featureStats = useMemo(() => {
    const available = features.filter(f => f.isAvailable).length;
    const unavailable = features.length - available;
    const categoryCount = new Set(features.map(f => f.category)).size;
    const totalMinutes = features.reduce((sum, f) => sum + (f.estimatedMinutes || 0), 0);
    const avgMinutes = features.length > 0 ? Math.round(totalMinutes / features.length) : 0;
    return { available, unavailable, categoryCount, avgMinutes };
  }, [features]);
    ...
    <div className="px-4 py-4 sm:px-6 sm:py-6 space-y-5 max-w-[1640px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 h-8 px-3 rounded-md text-[13px] font-medium border transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            style={{
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <ColoredStatsCard
          ...
          icon={Layers}
          color="purple"
          subtitle={`${filteredFeatures.length} shown`}
        />
        <ColoredStatsCard
          label="Available"
          value={featureStats.available}
          icon={CheckCircle}
          color="green"
          subtitle={`${featureStats.unavailable} unavailable`}
        />
        <ColoredStatsCard
          label="Categories"
          value={featureStats.categoryCount}
          icon={Database}
          color="blue"
        ...
        <ColoredStatsCard
          label="Avg. Time"
          value={`${featureStats.avgMinutes} min`}
          icon={Clock}
          color="amber"
          subtitle="Per feature"
        />
      </div>
      ...
      <QuickActionsGrid
        columns={3}
        actions={[
          { label: 'Scan New Feature', description: 'Identify capability from a jig or setup', icon: Plus, onClick: () => setViewMode('scanner') },
          { label: 'Export AI Context', description: 'Download features as JSON', icon: Download, onClick: handleExportAIContext },
          { label: 'Browse by Category', description: 'Filter by feature type', icon: Database, onClick: () => setCategoryFilter('all') },
        ]}
      />
