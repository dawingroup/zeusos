...
import { DEFAULT_USEFUL_LIFE, DEFAULT_SALVAGE_PERCENT } from '../services/depreciationService';
import type { Asset, AssetCategory, AssetStatus, DepreciationMethod } from '@/shared/types';
...
export function AssetForm({ asset, onSubmit, onCancel }: AssetFormProps) {
  ...
  const [purchaseDate, setPurchaseDate] = useState(
    asset?.financials?.purchaseDate || ''
  );
  const [supplier, setSupplier] = useState(asset?.financials?.supplier || '');
  const [invoiceReference, setInvoiceReference] = useState(asset?.financials?.invoiceReference || '');
  const [depreciationMethod, setDepreciationMethod] = useState<DepreciationMethod>(
    asset?.financials?.depreciationMethod || 'STRAIGHT_LINE'
  );
  const [usefulLifeYears, setUsefulLifeYears] = useState(
    asset?.financials?.usefulLifeYears ? String(asset.financials.usefulLifeYears) : String(DEFAULT_USEFUL_LIFE[category])
  );
  const [salvageValue, setSalvageValue] = useState(
    asset?.financials?.salvageValue ? String(asset.financials.salvageValue) : ''
  );
    ...
    try {
      ...
      if (!isNaN(price) && price > 0) {
        assetData.financials = {
          ...
          currency: 'UGX',
          depreciationMethod,
          usefulLifeYears: parseInt(usefulLifeYears) || DEFAULT_USEFUL_LIFE[category],
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setCategory(newCat);
                setUsefulLifeYears(String(DEFAULT_USEFUL_LIFE[newCat] || 5));
                if (purchasePrice) {
                  const price = parseFloat(purchasePrice);
                  if (!isNaN(price)) {
                    setSalvageValue(String(Math.round(price * (DEFAULT_SALVAGE_PERCENT[newCat] || 0))));
                  }
                }
              }}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (!isNaN(price)) {
                  setSalvageValue(String(Math.round(price * (DEFAULT_SALVAGE_PERCENT[category] || 0))));
                }
              }}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        ...
        {/* Depreciation Profile - shown when price is entered */}
        {purchasePrice && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <h4 className="text-sm font-semibold text-gray-700 mb-3">Depreciation Profile</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Method
