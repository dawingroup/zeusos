...
export function ShopifySettings() {
  const { status, isLoading, isConnecting, error, connect, disconnect } = useShopify();
  ...
  const handleConnect = async (e: React.FormEvent) => {
    e.preventDefault();
    const success = await connect(shopDomain, accessToken);
    if (success) {
      setShopDomain('');
      setAccessToken('');
      setShowForm(false);
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg border border-green-100">
              ...
              <div className="flex-1">
                <p className="font-medium text-green-800">Connected to {status.shopName}</p>
                <p className="text-sm text-green-600">{status.shopDomain}</p>
              </div>
              <a
