...
export interface UseShopifySyncReturn {
  ...
  updateProduct: (product: LaunchProduct) => Promise<{ success: boolean; error?: string }>;
  resyncProduct: (product: LaunchProduct) => Promise<{ success: boolean; error?: string }>;
...
export function useShopifySync(): UseShopifySyncReturn {
   ...
   */
  const updateProduct = useCallback(async (product: LaunchProduct): Promise<{ success: boolean; error?: string }> => {
    if (!isConnected) {
      return { success: false, error: 'Shopify is not connected' };
    }
    return shopifyService.updateShopifyProduct(product);
  }, [isConnected]);

  /**
   * Re-sync product data to Shopify (full rebuild)
   */
  const resyncProduct = useCallback(async (product: LaunchProduct): Promise<{ success: boolean; error?: string }> => {
    if (!isConnected) {
      return { success: false, error: 'Shopify is not connected' };
    }
    return shopifyService.resyncProduct(product);
  }, [isConnected]);
