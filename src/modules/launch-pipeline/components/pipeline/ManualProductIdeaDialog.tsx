/**
 * ManualProductIdeaDialog Component
 * Multi-step dialog for manually creating a product-idea clip via photo upload or URL.
 * Uses Google Search + Gemini AI to research and extract product details.
 * Adapted from design-manager's ManualClipDialog but saves as clipType: 'product-idea'.
 */
...
interface ManualProductIdeaDialogProps {
  open: boolean;
  onClose: () => void;
  userId: string;
  onClipCreated?: () => void;
...
export function ManualProductIdeaDialog({ open, onClose, userId, onClipCreated }: ManualProductIdeaDialogProps) {
  // Step state
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const clipData = {
        ...
        // AI analysis summary (for compatibility with ProductIdeaClips display)
        aiAnalysis: {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Clip Product Idea</h2>
            ...
            {(['input', 'researching', 'review', 'saving', 'done'] as Step[]).map((s, i) => {
              ...
              return (
                <div key={s} className="flex items-center flex-1">
                  {i > 0 && (
                    <div className={`flex-1 h-px ${currentIdx >= i ? 'bg-indigo-600' : 'bg-gray-200'}`} />
                  )}
                  <div className={`w-2 h-2 rounded-full ${step === s ? 'bg-indigo-600 scale-125' : currentIdx > i ? 'bg-indigo-600' : 'bg-gray-200'}`} />
                </div>
              );
            })}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="flex rounded-lg border border-gray-200 p-1 gap-1">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-md transition-colors ${
                    inputMode === 'photo' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-gray-50'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-md transition-colors ${
                    inputMode === 'url' ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  ...
                  {!previewUrl ? (
                    <div
                      ...
                      className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${
                        dragOver ? 'border-indigo-600 bg-indigo-50' : 'border-gray-300 hover:border-gray-400'
                      }`}
                    >
          ...
          {step === 'saving' && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              <p className="text-sm text-gray-500">Saving product idea...</p>
            </div>
          )}
          ...
          {step === 'done' && (
            <div className="space-y-4">
              <div className="flex flex-col items-center py-6">
                ...
                <p className="text-sm font-medium text-gray-700">Product idea clipped</p>
                <p className="text-xs text-gray-400 mt-1">
                  It's now ready to be converted into a Launch Pipeline product.
                </p>
              </div>
