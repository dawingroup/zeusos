/**
 * Product Detail Page
 * Full product view with inline AI tools, Shopify publishing, and stage history
 */
...
import { AIActionsPanel } from '../components/product/AIActionsPanel';
import { NamingWizard, DiscoverabilityPanel } from '../components/ai-assistant';
import { PrePublishDialog } from '../components/shopify/PrePublishDialog';
...
export default function ProductDetailPage() {
  ...
  const { isConnected, resyncProduct } = useShopifySync();
  ...
  const handlePublish = async () => {
    ...
    if (result.shopifyProductId) {
      await shopifyService.updateSyncState(product.id, {
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div className="flex items-start gap-3 sm:gap-4 min-w-0">
          ...
          <div>
            ...
            <p className="text-gray-500 capitalize break-words">
              {product.category} {'\u2022'} {product.handle || 'no-handle'}
              {product.pricing?.basePrice ? ` \u2022 ${product.pricing.currency} ${product.pricing.basePrice.toFixed(2)}` : ''}
            </p>
          </div>
        </div>
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {product.specifications.materials.map((mat, i) => (
                            <span key={i} className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{mat}</span>
                          ))}
                        </div>
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {product.specifications.finishes.map((fin, i) => (
                            <span key={i} className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{fin}</span>
                          ))}
                        </div>
                      </div>
                    )}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {product.specifications.features.map((feat, i) => (
                            <span key={i} className="px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700">{feat}</span>
                          ))}
                        </div>
                      </div>
                    )}
              ...
              {/* Pricing & Variants Summary */}
              {(product.pricing || product.variants?.length) && (
                <Card>
                  <CardHeader>
                    ...
                    {product.pricing && (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold">{product.pricing.currency} {product.pricing.basePrice.toFixed(2)}</p>
                        </div>
                        {product.pricing.compareAtPrice && (
                          <div>
                            <h4 className="text-sm font-medium text-gray-500">Compare At</h4>
                            <p className="text-lg font-semibold text-gray-400 line-through">{product.pricing.currency} {product.pricing.compareAtPrice.toFixed(2)}</p>
                          </div>
                        )}
                        {product.pricing.costPerItem && (
                          <div>
                            <h4 className="text-sm font-medium text-gray-500">Cost</h4>
                            <p className="text-lg font-semibold text-gray-600">{product.pricing.currency} {product.pricing.costPerItem.toFixed(2)}</p>
                          </div>
                        )}
                        {product.pricing.costPerItem && product.pricing.basePrice > 0 && (
                          <div>
                            <h4 className="text-sm font-medium text-gray-500">Margin</h4>
                            <p className={`text-lg font-semibold ${
                              ((product.pricing.basePrice - product.pricing.costPerItem) / product.pricing.basePrice * 100) > 0
                                ? 'text-green-600' : 'text-red-600'
                            }`}>
                              {((product.pricing.basePrice - product.pricing.costPerItem) / product.pricing.basePrice * 100).toFixed(1)}%
                            </p>
                          </div>
                        )}
                      </div>
                          ...
                          {product.variants.map(v => (
                            <div key={v.id} className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded">
                              ...
                              <div className="flex items-center gap-3 text-gray-500">
                                ...
                                <span>Qty: {v.inventoryQuantity}</span>
                                <span className="font-medium text-gray-900">${v.price.toFixed(2)}</span>
                              </div>
                            </div>
                          ))}
                        </div>
            ...
            {/* Right Column - AI Actions & Quick Actions */}
            <div className="space-y-6">
              {/* AI Enhancement Panel */}
              <AIActionsPanel product={product} />
                ...
                <CardContent>
                  {product.namingSession ? (
                    <div className="space-y-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => setShowNamingWizard(true)} className="w-full">
                        Generate New Names
                      </Button>
                    </div>
                  ) : (
                    <Button onClick={() => setShowNamingWizard(true)} size="sm" className="w-full">
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {product.tags.map((tag, i) => (
                        <span key={i} className="px-2 py-0.5 rounded-full text-xs font-medium border border-gray-200 text-gray-700">{tag}</span>
                      ))}
                    </div>
                  </CardContent>
                </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {product.shopifySync?.status === 'synced' ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 p-3 bg-green-50 rounded-lg">
                      ...
                      <div>
                        <p className="font-medium text-green-800">Published to Shopify</p>
                        <p className="text-sm text-green-600">
                          Product ID: {product.shopifySync.shopifyProductId}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t">
                      <p className="text-xs text-gray-500 mb-2">
                        Last synced: {product.shopifySync.lastSyncedAt?.toDate?.()?.toLocaleDateString() || 'Unknown'}
                      </p>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <div className="space-y-3">
                    <p className="text-sm text-gray-600">
                      This product has not been published to Shopify yet.
                    </p>
                    <Button
                  ...
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="text-sm text-gray-600">
                      Shopify is not connected. Go to the Shopify Store tab to connect your store.
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {[
                    { label: 'Product description', ok: !!(product.aiContent?.fullDescription || product.description) },
                    { label: 'Hero images', ok: product.deliverables?.some(d => d.type === 'hero_images' && d.url) },
                    { label: 'SEO metadata', ok: !!(product.seoMetadata?.metaTitle || product.aiContent?.metaDescription) },
                    { label: 'Pricing set', ok: !!(product.pricing?.basePrice && product.pricing.basePrice > 0) },
                    { label: 'Product dimensions', ok: !!product.specifications?.dimensions },
                    { label: 'Materials listed', ok: !!(product.specifications?.materials?.length) },
                  ].map((check, i) => (
