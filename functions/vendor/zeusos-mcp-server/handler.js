...
export async function createMcpHandler(req, res) {
    ...
    if (toolName) {
        ...
        if (!allowMutations && isMutatingTool(toolName)) {
            ...
            res.status(403).json(jsonRpcErrorResponse(rpc.id, -32000, `Tool "${toolName}" is blocked by compliance policy (set ZEUSOS_MCP_ALLOW_MUTATIONS=true to override).`));
            console.warn(`[zeusos-mcp] blocked mutating tool call tool=${toolName} ip=${ip} ua=${req.get('user-agent') ?? 'unknown'}`);
            return;
        }
    }
    await writeAuditLog({
    ...
    console.info(`[zeusos-mcp] request requestId=${requestId} actor=${actorId} method=${rpc.method ?? 'unknown'} tool=${toolName || '-'} argsHash=${toolName ? argsDigest : '-'} ip=${ip}`);
