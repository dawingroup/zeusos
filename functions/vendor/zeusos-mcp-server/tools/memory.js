...
export function registerMemoryTools(server) {
    ...
    // ─── zeusos_get_memory_context ───────────────────────────────────────────────
    server.registerTool('zeusos_get_memory_context', {
        title: 'Get Memory Context',
        description: `Retrieve relevant memories from ZeusOS's business memory store. Returns formatted context that can be injected into AI prompts. Useful for understanding historical decisions, customer preferences, and accumulated business knowledge.`,
        inputSchema: {
        ...
        try {
            const result = await callCloudFunction('getMemoryContext', {
            ...
            if (result.memories?.length) {
                for (const mem of result.memories) {
                    ...
                    if (mem.tags?.length)
                        lines.push(`  _Tags: ${mem.tags.join(', ')}_`);
                }
            }
            else {
    ...
    // ─── zeusos_save_memory ──────────────────────────────────────────────────────
    server.registerTool('zeusos_save_memory', {
        title: 'Save Business Memory',
        description: `Save a piece of business knowledge to ZeusOS's memory store. Use this to capture decisions, insights, customer preferences, market intelligence, or any fact that should inform future AI responses.`,
        inputSchema: {
        ...
        try {
            const result = await callCloudFunction('saveManualMemory', {
    ...
    // ─── zeusos_search_memory ────────────────────────────────────────────────────
    server.registerTool('zeusos_search_memory', {
        title: 'Semantic Memory Search',
        description: `Search the ZeusOS business memory store using semantic similarity. Returns memories most relevant to the query, ranked by similarity score.`,
        inputSchema: {
        ...
        try {
            const result = await callCloudFunction('semanticMemorySearch', {
                query: params.query,
            ...
            for (const r of result.results) {
                ...
                if (r.tags?.length)
                    lines.push(`_Tags: ${r.tags.join(', ')}_`);
                lines.push('');
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
    ...
    server.registerTool('zeusos_extract_memories', {
        ...
        try {
            const result = await callCloudFunction('extractMemories', {
            ...
            for (let i = 0; i < (result.memories?.length ?? 0); i++) {
                ...
                if (result.memoryIds?.[i])
                    lines.push(`   _ID: ${result.memoryIds[i]}_`);
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch (err) {
