...
- "Show me the status of all active projects"`,
        ...
        try {
            const result = await callCloudFunction('crossModuleIntelligence', {
    ...
    // ─── zeusos_strategy_research ───────────────────────────────────────────────
    server.registerTool('zeusos_strategy_research', {
        title: 'Strategy Research',
        description: `Conduct AI-powered strategy research on marketing-agency topics (FMCG, finance, telecom, energy, brand strategy, media planning). Performs web searches and analysis to surface trends, audience insights, channel mix, media rates, and competitor brand activity relevant to ZeusOS projects.
...
- competitors: Competitor landscape analysis`,
        ...
        try {
            const result = await callCloudFunction('strategyResearch', {
                query: params.query,
            ...
            const sections = [result.findings];
    ...
    // ─── zeusos_material_demand_forecast ────────────────────────────────────────
    server.registerTool('zeusos_material_demand_forecast', {
        ...
        try {
            const result = await callCloudFunction('materialDemandForecast', {
    ...
    // ─── zeusos_assess_strategy_section ─────────────────────────────────────────
    server.registerTool('zeusos_assess_strategy_section', {
        ...
        try {
            const result = await callCloudFunction('assessStrategySection', {
    ...
    server.registerTool('zeusos_rewrite_strategy_section', {
        ...
        try {
            const result = await callCloudFunction('rewriteStrategySection', {
                companyId: params.company_id,
                reviewId: params.review_id,
                sectionId: params.section_id,
