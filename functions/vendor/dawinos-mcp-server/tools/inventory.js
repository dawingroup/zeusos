import { z } from 'zod';
import { queryCollection, fetchAllMatching, getDb, serializeDoc, formatTimestamp, formatCurrency, truncateIfNeeded, serverTimestamp, FieldValue, } from '../services/firebase.js';
import { COLLECTIONS, DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE, INVENTORY_STATUSES, INVENTORY_TIERS, FINISH_CATEGORIES, } from '../constants.js';
import { getValidCategorySlugs, validateCategorySlug, } from '../services/categoryRegistry.js';
import { loadItem, deepEqual, emptyEnvelope, findSkuCollision, findBatchSkuCollisions, scanSkuReferences, commitBatch, } from '../inventory/write-helpers.js';
...
export function registerInventoryTools(server) {
    ...
    server.registerTool('dawinos_search_inventory', {
...
Archived items are excluded by default — pass include_archived=true to see them.`,
        ...
        try {
            const { query, category, classification, status, tier, restockable, currency, is_family, parent_id, include_archived, low_stock_only, limit, offset, compact, fields, response_format, } = params;
            // Validate category against dynamic registry
            if (category) {
                const catError = await validateCategorySlug(category);
                if (catError)
                    return { content: [{ type: 'text', text: catError }] };
            }
            ...
            if (classification)
                filters.push({ field: 'classification', op: '==', value: classification });
            if (status)
                filters.push({ field: 'status', op: '==', value: status });
            if (tier)
                filters.push({ field: 'tier', op: '==', value: tier });
            if (restockable !== undefined)
                filters.push({ field: 'restockable', op: '==', value: restockable });
            if (parent_id)
                filters.push({ field: 'parentItemId', op: '==', value: parent_id });
            if (is_family === true)
                filters.push({ field: 'isFamily', op: '==', value: true });
            ...
            const allMatching = await fetchAllMatching(COLLECTIONS.INVENTORY_ITEMS, {
                filters,
            });
            ...
            const getCost = (i) => typeof i.pricing?.costPerUnit === 'number' ? i.pricing.costPerUnit : i.costPrice;
            const getCurrency = (i) => i.pricing?.currency ?? i.currency;
            const getUnit = (i) => i.pricing?.unit ?? i.unit;
            ...
            if (compact) {
                ...
                for (const item of paginated) {
                    ...
                    lines.push(`| \`${item.id}\` | ${item.name ?? 'N/A'}${flag} | ${item.sku ?? '-'} | ${item.category ?? '-'} | ${stock} | ${reorder || '-'} | ${formatCurrency(cost, cur)} | ${unit} |`);
                }
            }
            if (hasMore) {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error searching inventory: ${error.message}` }],
            };
        }
    });
    // ─── Tool 2: Get Finish Library ───────────────────────────────────────────────
    server.registerTool('dawinos_get_finish_library', {
...
Returns: Finish name, code, category, subtype, hex color, pattern type, availability, and tags.`,
        ...
        try {
            const { category, subtype, availability, search, is_active, limit, offset, response_format } = params;
            // Validate category against dynamic registry
            if (category) {
                const catError = await validateCategorySlug(category);
                if (catError)
            ...
            if (category)
                filters.push({ field: 'category', op: '==', value: category });
            if (availability)
                filters.push({ field: 'availability', op: '==', value: availability });
            if (is_active)
                filters.push({ field: 'isActive', op: '==', value: true });
            const { items, total } = await queryCollection(COLLECTIONS.FINISH_LIBRARY, {
                filters,
            ...
            const paginated = search || subtype ? filtered.slice(offset, offset + limit) : filtered.slice(0, limit);
            const hasMore = search || subtype
            ...
            if (response_format === 'json') {
                const result = {
                    ...
                    limit,
                    hasMore,
                    items: paginated.map((f) => serializeDoc(f)),
                };
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            const lines = [
            ...
            for (const f of paginated) {
                ...
                lines.push(`| ${f.code ?? '-'} | ${f.name ?? 'N/A'} | ${f.category ?? '-'} | ${f.subtype ?? '-'} | ${colorSwatch} | ${f.patternType ?? '-'} | ${f.availability ?? '-'} |`);
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error fetching finish library: ${error.message}` }],
            };
        }
    });
    // ─── Tool 3: Stock Levels ─────────────────────────────────────────────────────
    server.registerTool('dawinos_stock_levels', {
...
Fields are: quantityOnHand, quantityAllocated, quantityAvailable (not 'stockOnHand').`,
        ...
        try {
            const { inventory_item_id, warehouse_id, low_stock_only, limit, offset, response_format } = params;
            const filters = [];
            if (inventory_item_id)
                filters.push({ field: 'inventoryItemId', op: '==', value: inventory_item_id });
            if (warehouse_id)
                filters.push({ field: 'warehouseId', op: '==', value: warehouse_id });
            const { items, total } = await queryCollection(COLLECTIONS.STOCK_LEVELS, {
                filters,
            ...
            if (response_format === 'json') {
                const result = {
                    ...
                    limit,
                    hasMore,
                    items: paginated.map((s) => serializeDoc(s)),
                };
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            const lines = [
            ...
            for (const s of paginated) {
                const isLow = (s.reorderPoint ?? 0) > 0 && (s.quantityAvailable ?? 0) <= (s.reorderPoint ?? 0);
                const flag = isLow ? ' ⚠️' : '';
                const extras = [];
                const inv = s['inventory'];
                if (inv?.['volumeOnHand'] != null)
                    extras.push(`vol: ${inv['volumeOnHand']}m³`);
                if (inv?.['piecesOnHand'] != null)
                    extras.push(`pcs: ${inv['piecesOnHand']}`);
                const extraStr = extras.length > 0 ? ` (${extras.join(', ')})` : '';
                lines.push(`| ${s.itemName ?? s.inventoryItemId ?? 'N/A'}${flag} | ${s.warehouseId ?? '-'} | ${s.quantityOnHand ?? 0}${extraStr} | ${s.quantityAllocated ?? 0} | ${s.quantityAvailable ?? 0} | ${s.reorderPoint ?? '-'} |`);
            }
            if (hasMore) {
                lines.push(`\n_Use offset=${offset + limit} for next page._`);
            }
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error fetching stock levels: ${error.message}` }],
            };
        }
    });
    // ─── Tool 4: List Stock Adjustments ──────────────────────────────────────────
    server.registerTool('dawinos_list_stock_adjustments', {
...
Note: Collection is 'stock_adjustments' (snake_case). Line items are embedded in the document.`,
        ...
        try {
            const { status, adjustment_type, date_from, date_to, limit, offset, response_format } = params;
            const filters = [];
            if (status)
                filters.push({ field: 'status', op: '==', value: status });
            if (adjustment_type)
                filters.push({ field: 'adjustmentType', op: '==', value: adjustment_type });
            const { items, total } = await queryCollection(COLLECTIONS.STOCK_ADJUSTMENTS, {
                filters,
            ...
            const paginated = date_from || date_to ? filtered.slice(offset, offset + limit) : filtered.slice(0, limit);
            const hasMore = date_from || date_to
                ? offset + paginated.length < filtered.length
                : offset + paginated.length < total;
            if (paginated.length === 0) {
                return {
            ...
            if (response_format === 'json') {
                const result = {
                    ...
                    limit,
                    hasMore,
                    items: paginated.map((a) => serializeDoc(a)),
                };
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            const lines = [
            ...
            for (const adj of paginated) {
                ...
                lines.push(`| ${adj.adjustmentNumber ?? adj.id.slice(0, 8)} | ${adj.adjustmentType ?? '-'} | ${adj.status ?? '-'} | ${lineCount} | ${formatCurrency(adj.netValueImpact)} | ${adj.createdBy ?? '-'} | ${formatTimestamp(adj.createdAt)} |`);
            }
            if (hasMore) {
                lines.push(`\n_Use offset=${offset + limit} for next page._`);
            }
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error listing stock adjustments: ${error.message}` }],
            };
        }
    });
    // ─── Tool 5: Create Inventory Item ───────────────────────────────────────────
    server.registerTool('dawinos_create_inventory_item', {
...
Common unit values: pcs, m2, m, lm, kg, L, set.`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error creating inventory item: ${err.message}` }],
            };
        }
    });
    // ─── Tool 6: Update Inventory Item ───────────────────────────────────────────
    server.registerTool('dawinos_update_inventory_item', {
...
Returns a WriteEnvelope: planned/committed counts, the full diff, warnings, errors.`,
        inputSchema: {
            ...
            allow_cross_ref: z
                .boolean()
                .default(false)
                .describe('If true, cross-reference warnings on SKU rename are demoted to info (still surfaced, not blocking). Default false = warnings block commit.'),
        },
        annotations: {
            readOnlyHint: false,
        ...
        const env = emptyEnvelope(params.dry_run ? 'dry_run' : 'commit');
        try {
            // Validate category against dynamic registry
            ...
            const update = {};
            const changes = [];
            const existingPricing = before['pricing'] || {};
            const existingInventory = before['inventory'] || {};
            const recordChange = (path, prev, next) => {
            ...
            if (params.primary_supplier !== undefined &&
            ...
            if (params.sku !== undefined && params.sku !== beforeSku) {
                ...
                if (xrefWarnings.length > 0 && !params.allow_cross_ref) {
                    env.errors.push(`SKU rename blocked by cross-reference warnings. Pass allow_cross_ref: true to proceed.`);
                    return { content: [{ type: 'text', text: formatEnvelope(env, toolName) }] };
                }
            }
            const diff = {
            ...
            env.errors.push(`Error: ${err.message}`);
            return {
                isError: true,
    ...
    server.registerTool('dawinos_create_stock_adjustment', {
...
Each line item needs: inventory_item_id, item_name, adjustment_type (increase|decrease), quantity, unit_cost.`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error creating stock adjustment: ${err.message}` }],
            };
        }
    });
    // ─── Tool 8: Delete Inventory Item ───────────────────────────────────────────
    server.registerTool('dawinos_delete_inventory_item', {
...
Requires confirmation: pass confirm=true to execute. Without it, returns a dry-run summary.`,
        ...
        try {
            ...
            const name = data['name'] ?? params.item_id;
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error deleting inventory item: ${err.message}` }],
            };
        }
    });
    // ─── Tool 9: Batch Update Inventory ──────────────────────────────────────────
    server.registerTool('dawinos_batch_update_inventory', {
...
Returns a summary of how many succeeded or failed.`,
        ...
        try {
            ...
            const catsToCheck = params.updates.map(u => u.category).filter(Boolean);
            ...
            for (const u of params.updates) {
                ...
                if (u.purchase_uom !== undefined) {
                    update['purchaseUom'] = u.purchase_uom;
                    changed.push('purchaseUom');
                }
                if (u.stock_uom !== undefined) {
                    update['stockUom'] = u.stock_uom;
                    changed.push('stockUom');
                }
                ...
                if (u.primary_supplier !== undefined) {
                    update['preferredSupplierName'] = u.primary_supplier;
                    changed.push('preferredSupplierName');
                }
                if (u.tags !== undefined) {
                    update['tags'] = u.tags;
                    changed.push('tags');
                ...
                if (u.restockable !== undefined) {
                    update['restockable'] = u.restockable;
                    changed.push('restockable');
                }
                if (u.project_ids !== undefined) {
                    update['linkedProjectIds'] = u.project_ids;
                    changed.push('linkedProjectIds');
                }
                if (changed.length === 0) {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error in batch update: ${err.message}` }],
            };
        }
    });
    // ─── Tool 10: Set Family Definitions ─────────────────────────────────────────
    server.registerTool('dawinos_set_family_definitions', {
...
Returns a WriteEnvelope with the diff.`,
        ...
        try {
            ...
            if (!item.data['isFamily']) {
                env.errors.push(`Item ${params.item_id} is not a family parent (isFamily !== true). Refusing to set variantAttributeDefinitions.`);
                return { content: [{ type: 'text', text: formatEnvelope(env, toolName) }] };
            }
            ...
            const before = item.data['variantAttributeDefinitions'] || [];
            let after;
            if (hasReplace) {
            ...
            const result = await commitBatch([{ ref: item.ref, update: { variantAttributeDefinitions: after } }], toolName);
            env.itemsCommitted = result.committed;
            env.errors.push(...result.errors);
            return { content: [{ type: 'text', text: formatEnvelope(env, toolName) }] };
        }
    ...
    server.registerTool('dawinos_set_variant_attributes', {
...
Returns a WriteEnvelope with the diff.`,
        ...
        try {
            ...
            const beforeAttrs = item.data['variantAttributes'] || [];
            const beforeTags = item.data['parametricTags'] || {};
            ...
            if (params.parametric_tags !== undefined) {
                ...
                afterTags = {};
                for (const a of afterAttrs)
                    afterTags[a.key] = a.value;
            }
            else {
    ...
    server.registerTool('dawinos_update_family_membership', {
...
Returns a WriteEnvelope showing both doc diffs.`,
        ...
        try {
            ...
            const beforeIds = parent.data['familySkuIds'] || [];
            const beforeFamilyId = child.data['familyId'] || '';
            const beforeParentItemId = child.data['parentItemId'] || '';
            let afterIds;
            let afterChildFamilyId;
            let afterChildParentItemId;
            if (params.action === 'attach') {
                if (beforeFamilyId && beforeFamilyId !== params.parent_id && !params.allow_reparent) {
                    env.errors.push(`Child ${params.child_id} already belongs to family ${beforeFamilyId}. Pass allow_reparent: true to move it.`);
                    return { content: [{ type: 'text', text: formatEnvelope(env, toolName) }] };
                }
    ...
    server.registerTool('dawinos_batch_inventory_rewrite', {
...
Returns a WriteEnvelope with all per-item diffs, collision warnings, and errors.`,
        ...
        try {
            // Validate all categories upfront
            const catsToCheck = params.patches.map((p) => p.category).filter(Boolean);
            if (catsToCheck.length > 0) {
                const validSlugs = await getValidCategorySlugs();
                const invalid = [...new Set(catsToCheck)].filter(c => !validSlugs.includes(c));
                if (invalid.length > 0) {
            ...
            const loaded = await Promise.all(params.patches.map(async (p) => ({ patch: p, item: await loadItem(p.item_id) })));
            const writes = [];
            const skuRenames = [];
            for (const { patch, item } of loaded) {
                ...
                if (patch.preferred_supplier_name !== undefined &&
                ...
                if (patch.cost_price !== undefined || patch.currency !== undefined) {
                    const existingPricing = before['pricing'] || {};
                    const newPricing = { ...existingPricing };
                    if (patch.cost_price !== undefined)
                        newPricing['costPerUnit'] = patch.cost_price;
                    if (patch.currency !== undefined)
                        newPricing['currency'] = patch.currency;
            ...
            if (skuRenames.length > 0) {
                ...
                if (collisions.length > 0) {
                    env.errors.push(`Batch aborted: ${collisions.length} SKU collision(s). No writes performed.`);
                    return { content: [{ type: 'text', text: formatEnvelope(env, toolName) }] };
                }
            }
            if (params.dry_run) {
