import { z } from 'zod';
import { queryCollection, getDb, formatCurrency, formatTimestamp, } from '../services/firebase.js';
...
export function registerFinanceTools(server) {
    ...
    // ─── dawinos_expenditure_queue ────────────────────────────────────────────────
    server.registerTool('dawinos_expenditure_queue', {
...
Shows which bills/payments should be paid today, which can be deferred, and why. Essential for cash management decisions.`,
        ...
        try {
            ...
            if (params.tier)
                filters.push({ field: 'tier', op: '==', value: params.tier });
            const { items, total } = await queryCollection(COMPANY_PATHS.expenditureQueue(), {
                filters,
            ...
            if (!items.length) {
                return { content: [{ type: 'text', text: 'No expenditures in queue.' }] };
            }
            ...
            for (const d of items) {
                ...
                const dueDate = d.urgency?.latestDate
                    ...
                    : '—';
                lines.push(`| ${d.tier ?? '—'} | ${d.type ?? '—'} | ${formatCurrency(Number(d.amountUGX) || 0, 'UGX')} | ${dueDate} | ${score} | ${revUnlock} | ${d.status ?? '—'} |`);
            }
    ...
    server.registerTool('dawinos_run_cash_flow_scenario', {
...
Returns: baseline vs modified cash position, risk level, insights, tradeoffs, recommendations.`,
        ...
        try {
            const result = await callCloudFunction('runCashFlowScenario', {
                companyId,
                scenario: {
    ...
    server.registerTool('dawinos_generate_spend_plan', {
...
After generating, use \`dawinos_get_spend_plan\` (without an ID) to see the full scheduled vs deferred breakdown.`,
        inputSchema: {
        ...
        try {
            const result = await callCloudFunction('generateSpendPlan', {
                companyId,
            ...
            return {
                content: [{
                        ...
                        text: [
                            ...
                            '',
                            `Run \`dawinos_get_spend_plan\` to see the full payment schedule and deferred items.`,
                        ].join('\n'),
                    }],
            };
        }
    ...
    // ─── dawinos_get_spend_plan ───────────────────────────────────────────────────
    server.registerTool('dawinos_get_spend_plan', {
        ...
        try {
            ...
            if (!plan) {
                const { items } = await queryCollection(COMPANY_PATHS.spendPlans(), {
            ...
            if (!plan) {
                return { content: [{ type: 'text', text: 'No spend plan found. Use `dawinos_generate_spend_plan` to create one.' }] };
            }
            ...
            if (scheduled.length) {
                ...
                for (const [i, item] of scheduled.entries()) {
                    lines.push(`| ${i + 1} | ${String(item['description'] ?? '').substring(0, 40)} | ${formatCurrency(Number(item['amount']) || 0, 'UGX')} | ${item['priorityTier']} | ${Number(item['compositeScore']).toFixed(2)} |`);
                }
                lines.push('');
            }
            if (deferred.length) {
                ...
                if (deferred.length > 10)
                    lines.push(`  _...and ${deferred.length - 10} more_`);
                lines.push('');
            }
            ...
            if (actions.length) {
                ...
                lines.push(...actions.map((a) => `• ${String(a)}`));
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch (err) {
    ...
    // ─── dawinos_market_intelligence ─────────────────────────────────────────────
    server.registerTool('dawinos_market_intelligence', {
        title: 'Market Intelligence Reports',
        description: `Retrieve AI-generated market intelligence reports for DawinOS subsidiaries. Each report contains competitor analyses, threat assessments, emerging industry trends, and strategic pricing/positioning recommendations based on live web data.
...
Subsidiary IDs: finishes | advisory | technology | capital`,
        ...
        try {
            const result = await callCloudFunction('getMarketIntelligenceReports', {
                subsidiaryId: params.subsidiary_id,
                limit: params.limit ?? 3,
            });
            if (!result.reports?.length) {
                return { content: [{ type: 'text', text: `No market intelligence reports found for "${params.subsidiary_id}".` }] };
            }
            const lines = [];
            for (const report of result.reports) {
                lines.push(`# ${report.reportTitle}`, `Generated: ${report.generatedAt}  |  ${report.subsidiaryName}`, `Threat Level: **${report.overallThreatLevel}**  |  Sentiment: ${report.marketSentiment}`, `${report.metadata?.competitorsAnalyzed ?? 0} competitors  |  ${report.metadata?.totalFindings ?? 0} findings  |  Confidence: ${((report.metadata?.confidenceScore ?? 0) * 100).toFixed(0)}%`, '', '## Executive Summary', report.executiveSummary, '');
                if (report.competitorAnalyses?.length) {
                    ...
                    for (const comp of report.competitorAnalyses) {
                        ...
                        if (comp.watchItems?.length)
                            lines.push(`  Watch: ${comp.watchItems.join(', ')}`);
                    }
                    lines.push('');
                }
                ...
                if (Array.isArray(recs) && recs.length) {
                    ...
                    lines.push(...recs.map((r) => `→ ${String(r)}`));
                    lines.push('');
                }
