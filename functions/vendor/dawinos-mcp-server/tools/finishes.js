...
import { COLLECTIONS, DEFAULT_ORG_ID, FINISH_CATEGORIES, } from '../constants.js';
const AVAILABILITY_VALUES = ['in_stock', 'made_to_order', 'discontinued', 'seasonal'];
const PATTERN_VALUES = [
    'solid', 'woodgrain', 'marble', 'textile', 'geometric', 'speckled', 'custom',
];
const SOURCE_VALUES = ['manual', 'supplier_catalogue', 'imported'];
 ...
 */
function buildDocData(entry, timestamps) {
    const data = {
    ...
    if (entry.hex !== undefined)
        data.hexColor = entry.hex;
    if (entry.secondary_hex !== undefined)
        data.secondaryColor = entry.secondary_hex;
    if (entry.pattern !== undefined)
        data.patternType = entry.pattern;
    if (entry.notes !== undefined)
        data.notes = entry.notes;
    if (entry.thumbnail_url !== undefined)
        data.thumbnailUrl = entry.thumbnail_url;
    if (entry.cost_modifier !== undefined)
        data.costModifier = entry.cost_modifier;
    if (entry.cost_modifier_type !== undefined)
        data.costModifierType = entry.cost_modifier_type;
    if (entry.cost_modifier_unit !== undefined)
        data.costModifierUnit = entry.cost_modifier_unit;
    if (entry.primary_supplier !== undefined)
        data.primarySupplier = entry.primary_supplier;
 ...
 */
async function findMissingInventoryIds(ids) {
    if (ids.length === 0)
        return [];
    const db = getDb();
    const results = await Promise.all(ids.map((id) => db.collection(COLLECTIONS.INVENTORY_ITEMS).doc(id).get()));
    return ids.filter((_, i) => !results[i].exists);
}
async function findDuplicateCode(code, excludeId) {
    const db = getDb();
    const snap = await db
...
// ─── Tool registrations ──────────────────────────────────────────────────────
export function registerFinishTools(server) {
    // ─── Create ────────────────────────────────────────────────────────────────
    server.registerTool('dawinos_create_finish_library_entry', {
...
missing IDs surface as a warning but do not block creation.`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error creating finish: ${err.message}` }],
            };
        }
    });
    // ─── Update ────────────────────────────────────────────────────────────────
    server.registerTool('dawinos_update_finish_library_entry', {
...
triggers a uniqueness pre-check that aborts on collision.`,
        ...
        try {
            const { finish_id, dry_run, ...updates } = params;
            const db = getDb();
            ...
            if (Array.isArray(updates.linked_inventory_ids)) {
                const missing = await findMissingInventoryIds(updates.linked_inventory_ids);
                if (missing.length > 0) {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error updating finish: ${err.message}` }],
            };
        }
    });
    // ─── Delete ────────────────────────────────────────────────────────────────
    server.registerTool('dawinos_delete_finish_library_entry', {
...
it by code.`,
        ...
        try {
            const { finish_id, hard_delete } = params;
            const db = getDb();
            const ref = db.collection(COLLECTIONS.FINISH_LIBRARY).doc(finish_id);
            const snap = await ref.get();
            if (!snap.exists) {
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error deleting finish: ${err.message}` }],
            };
        }
    });
    // ─── Bulk import ───────────────────────────────────────────────────────────
    server.registerTool('dawinos_bulk_import_finishes', {
...
atomically, so either all rows land or none do.`,
        ...
        try {
            const { entries, default_source } = params;
            ...
            const db = getDb();
            const existing = await Promise.all(entries.map((e) => findDuplicateCode(e.code)));
            ...
            if (missingIds.size > 0) {
                lines.push(``, `### Warnings`, `- ⚠️ Unknown linked inventory IDs referenced by one or more rows: ${Array.from(missingIds).join(', ')}`);
            }
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch (err) {
            return {
                isError: true,
                content: [{ type: 'text', text: `Error in bulk import: ${err.message}` }],
            };
        }
    });
    // ─── Link / unlink inventory ───────────────────────────────────────────────
    server.registerTool('dawinos_link_finish_to_inventory', {
...
sheet). Non-destructive — won't touch the legacy singular \`inventoryItemId\`.`,
        ...
        try {
            const { finish_id, add, remove } = params;
            ...
            const current = Array.isArray(snap.data()?.linkedInventoryItemIds)
                ? snap.data().linkedInventoryItemIds
                : [];
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Error updating finish links: ${err.message}` }],
            };
        }
    });
}
