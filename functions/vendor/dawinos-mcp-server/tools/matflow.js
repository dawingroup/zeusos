...
export function registerMatflowTools(server) {
...
Categories: CONCRETE, STEEL, MASONRY, TIMBER, ROOFING, PLUMBING, ELECTRICAL, FINISHES, DOORS_WINDOWS, HARDWARE, AGGREGATES, EARTHWORKS, OTHER`,
        ...
        try {
            ...
            const { items, total } = await queryCollection(MATFLOW_PATHS.formulas, {
                filters,
            ...
            if (!items.length) {
                return { content: [{ type: 'text', text: 'No formulas found.' }] };
            }
            const lines = [
            ...
            for (const f of items) {
                lines.push(`| ${f.code ?? '—'} | ${f.name} | ${f.category} | ${f.outputUnit} | ${f.components?.length ?? 0} |`);
            }
...
Provide either \`formula_id\` (Firestore doc ID) or \`code\` (e.g. "C25", "BRICK_230").`,
        ...
        if (!params.formula_id && !params.code) {
            return { isError: true, content: [{ type: 'text', text: 'Provide either formula_id or code.' }] };
        }
        try {
            let formula = null;
            ...
            if (!formula) {
                return { content: [{ type: 'text', text: `Formula not found.` }] };
            }
            const lines = [
            ...
            for (const c of (formula.components ?? [])) {
                ...
                lines.push(`| ${c.materialName} | ${c.quantity} | ${c.unit} | ${c.wastagePercent ?? 0}% | ${net.toFixed(3)} ${c.unit} |`);
            }
            // Rates
            const rateLines = [];
            if (formula.defaultLaborRate)
                rateLines.push(`Labor: UGX ${formula.defaultLaborRate.toLocaleString()}/${formula.outputUnit}`);
            if (formula.defaultMaterialRate)
                rateLines.push(`Material: UGX ${formula.defaultMaterialRate.toLocaleString()}/${formula.outputUnit}`);
            if (formula.defaultEquipmentRate)
                rateLines.push(`Equipment: UGX ${formula.defaultEquipmentRate.toLocaleString()}/${formula.outputUnit}`);
            if (rateLines.length) {
                lines.push('', '**Default Rates:**', ...rateLines);
            }
            // Formulas
            if (formula.laborFormula)
                lines.push('', `Labor formula: \`${formula.laborFormula}\``);
            if (formula.materialFormula)
                lines.push(`Material formula: \`${formula.materialFormula}\``);
            if (formula.equipmentFormula)
                lines.push(`Equipment formula: \`${formula.equipmentFormula}\``);
...
Examples: "concrete grade 25", "brick wall", "rebar", "plaster", "roofing sheet"`,
        ...
        try {
            ...
            if (!primaryWord) {
                return { isError: true, content: [{ type: 'text', text: 'Query too short — provide at least one word with 3+ characters.' }] };
            }
            const filters = [
                { field: 'isActive', op: '==', value: true },
            ...
            if (!results.length) {
                return { content: [{ type: 'text', text: `No formulas found for "${params.query}".` }] };
            }
            const lines = [
            ...
            for (const f of results) {
                lines.push(`| ${f.code ?? '—'} | ${f.name} | ${f.category} | ${f.outputUnit} | ${f.usageCount ?? 0}× |`);
            }
...
Use \`dawinos_search_formulas\` or \`dawinos_list_formulas\` to find the right formula code first.`,
        ...
        try {
            ...
            for (const c of components) {
                ...
                const purchaseQty = c.moq
                    ? Math.ceil(netQty / c.moq) * c.moq
                    : netQty;
                const purchaseCell = purchaseQty !== netQty
                    ? `**${purchaseQty.toFixed(2)}** _(MOQ ${c.moq}${c.packUnit ? ' ' + c.packUnit : ''})_`
                    : netQty.toFixed(2);
                lines.push(`| ${c.materialName} | ${c.quantity} | ${c.wastagePercent ?? 0}% (+${wastageAdded.toFixed(2)}) | ${netQty.toFixed(2)} | ${purchaseCell} | ${c.unit} | ${costCell} |`);
            }
            ...
            lines.push('', `_Formula: ${formula.id} | Updated: ${formatTimestamp(formula.updatedAt)}_`, `_Note: Rates are default estimates. Adjust for site conditions and current market prices._`);
            return { content: [{ type: 'text', text: truncateIfNeeded(lines.join('\n')) }] };
        }
        catch (err) {
            return { isError: true, content: [{ type: 'text', text: `Error: ${String(err)}` }] };
...
Provide a unique \`code\` (e.g. "C30_PUMPED", "BLOCK_200_PLASTERED"), a category, output unit, and at least one component.`,
        ...
        try {
            ...
            const { items: existing } = await queryCollection(MATFLOW_PATHS.formulas, {
                filters: [{ field: 'code', op: '==', value: code }],
                limit: 1,
            });
            if (existing.length) {
                return { isError: true, content: [{ type: 'text', text: `Formula code "${code}" already exists (ID: ${existing[0].id}). Use dawinos_update_formula to modify it.` }] };
            }
...
To replace all components, supply the full new \`components\` array. To deactivate a formula use \`is_active: false\`.`,
        ...
        try {
            ...
            if (!formulaId && params.code) {
                ...
                if (!items.length) {
                    return { isError: true, content: [{ type: 'text', text: `Formula "${params.code}" not found.` }] };
                }
            ...
            if (params.output_unit !== undefined)
                updates['outputUnit'] = params.output_unit;
            if (params.category !== undefined)
                updates['category'] = params.category;
            if (params.default_labor_rate !== undefined)
                updates['defaultLaborRate'] = params.default_labor_rate;
            if (params.default_material_rate !== undefined)
                updates['defaultMaterialRate'] = params.default_material_rate;
            if (params.default_equipment_rate !== undefined)
                updates['defaultEquipmentRate'] = params.default_equipment_rate;
            if (params.keywords !== undefined)
                updates['keywords'] = params.keywords;
            if (params.is_active !== undefined)
                updates['isActive'] = params.is_active;
            ...
            const db = getDb();
            await db.collection(MATFLOW_PATHS.formulas).doc(formulaId).update(updates);
...
//# sourceMappingURL=matflow.js.map