...
function renderTree(nodes, indent = 0) {
    ...
    for (const node of nodes) {
        ...
        if (node.isSystem)
            flags.push('system');
        if (!node.allowItems)
            flags.push('group-only');
        if (node.expenseOnIssue)
            flags.push('expense-on-issue');
...
export function registerCategoryTools(server) {
    // ─── List Categories ──────────────────────────────────────────────────────
    server.registerTool('dawinos_list_categories', {
...
Use include_inactive=true to see disabled categories.`,
        ...
        try {
            const db = getDb();
            let query = db.collection(COLLECTION).orderBy('sortOrder');
    ...
    // ─── Create Category ──────────────────────────────────────────────────────
    server.registerTool('dawinos_create_category', {
...
The slug is auto-generated from the name if omitted. dry_run defaults to true — set to false to commit.`,
    ...
    // ─── Update Category ──────────────────────────────────────────────────────
    server.registerTool('dawinos_update_category', {
