...
export function registerAdvisoryAiTools(server) {
    const companyId = getRequiredCompanyId();
    // ─── dawinos_procurement_advisor ─────────────────────────────────────────────
    server.registerTool('dawinos_procurement_advisor', {
...
- full-analysis: Run both enrichment and analysis in a single call`,
        ...
        try {
            const result = await callCloudFunction('procurementAdvisor', {
            ...
            if (result.enrichment) {
                ...
                if (e.contactEmail)
                    lines.push(`Email: ${e.contactEmail}`);
                if (e.contactPhone)
                    lines.push(`Phone: ${e.contactPhone}`);
                if (e.address)
                    lines.push(`Address: ${e.address}`);
                if (e.capabilities?.length)
                    lines.push(`Capabilities: ${e.capabilities.join(', ')}`);
                if (e.leadTimeDays)
                    lines.push(`Lead time: ${e.leadTimeDays} days`);
                if (e.paymentTerms)
                    lines.push(`Payment terms: ${e.paymentTerms}`);
                lines.push('');
            }
            if (result.analysis) {
                const a = result.analysis;
                lines.push('**Purchase History Analysis:**');
                if (a.totalSpend != null)
                    lines.push(`Total spend: ${a.currency ?? 'UGX'} ${a.totalSpend?.toLocaleString()}`);
                if (a.orderCount != null)
                    lines.push(`Orders: ${a.orderCount}`);
                if (a.avgOrderValue != null)
                    lines.push(`Avg order: ${a.currency ?? 'UGX'} ${a.avgOrderValue?.toLocaleString()}`);
                if (a.onTimeDeliveryRate != null)
                    lines.push(`On-time delivery: ${(a.onTimeDeliveryRate * 100).toFixed(0)}%`);
                if (a.trends?.length)
    ...
    // ─── dawinos_cfo_briefing ─────────────────────────────────────────────────────
    server.registerTool('dawinos_cfo_briefing', {
        title: 'CFO Briefing',
        description: `Generate a comprehensive CFO-level financial briefing for DawinOS. Analyses cash position, committed expenditure, revenue pipeline, risk exposure, and provides strategic recommendations.
...
Returns: executive summary, key decisions needed, risk alerts, actionable recommendations, and a cash outlook narrative.`,
        ...
        try {
            const result = await callCloudFunction('generateCFOBriefing', { companyId });
            ...
            if (ctx) {
                lines.push('## Snapshot');
                if (ctx.activePOs != null)
                    lines.push(`- Active POs: ${ctx.activePOs}`);
                if (ctx.activeMOs != null)
                    lines.push(`- Active Manufacturing Orders: ${ctx.activeMOs}`);
                if (ctx.pendingApprovals != null)
    ...
    server.registerTool('dawinos_process_receipt', {
...
- efris: Uganda Revenue Authority EFRIS invoice — extracts tax fields, TIN, invoice number`,
        ...
        try {
            const result = await callCloudFunction('processReceiptOCR', {
            ...
            if (result.amount != null)
                lines.push(`Amount: ${result.currency ?? 'UGX'} ${result.amount.toLocaleString()}`);
            if (result.date)
                lines.push(`Date: ${result.date}`);
            if (result.referenceNumber)
                lines.push(`Reference: ${result.referenceNumber}`);
            if (result.invoiceNumber)
                lines.push(`Invoice #: ${result.invoiceNumber}`);
            if (result.tin)
                lines.push(`TIN: ${result.tin}`);
    ...
    // ─── dawinos_assistant_chat ───────────────────────────────────────────────────
    server.registerTool('dawinos_assistant_chat', {
        title: 'DawinOS Assistant Chat',
        description: `Chat with the DawinOS AI assistant. The assistant has access to business memory and can answer questions in different expert modes.
...
- document_expert: Document analysis, drafting, and review`,
        ...
        try {
            const result = await callCloudFunction('assistantChat', {
            ...
            const meta = [];
            if (result.memoryCount)
                meta.push(`${result.memoryCount} memories loaded`);
            if (result.mode)
                meta.push(`mode: ${result.mode}`);
            if (result.conversationId)
                meta.push(`conv: ${result.conversationId}`);
            const footer = meta.length ? `\n\n_${meta.join(' | ')}_` : '';
            return {
                content: [{ type: 'text', text: `${result.response}${footer}` }],
            };
        }
        catch (err) {
            return { isError: true, content: [{ type: 'text', text: `Error: ${String(err)}` }] };
...
//# sourceMappingURL=advisory-ai.js.map