import { z } from 'zod';
import { queryCollection, getDocument, formatCurrency, formatTimestamp, truncateIfNeeded, } from '../services/firebase.js';
import { COLLECTIONS, DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE } from '../constants.js';
const QUOTE_STATUSES = ['draft', 'approved', 'synced', 'invoiced'];
export function registerQuoteTools(server) {
    // ─── dawinos_list_quotes ──────────────────────────────────────────────────────
    server.registerTool('dawinos_list_quotes', {
...
- Analyse quote conversion (draft → approved → invoiced)`,
        ...
        try {
            ...
            if (params.customer_id)
                filters.push({ field: 'customerId', op: '==', value: params.customer_id });
            if (params.project_id)
                filters.push({ field: 'projectId', op: '==', value: params.project_id });
            const { items: quotes, total } = await queryCollection(COLLECTIONS.CLIENT_QUOTES, {
                filters,
            ...
            for (const [i, q] of quotes.entries()) {
                ...
                lines.push(`| ${i + 1} | ${(q.customerId ?? '—').substring(0, 20)} | ${q.status ?? '—'} | ${formatCurrency(Number(q.totalPrice) || 0, currency)} | ${itemCount} | ${qbo} | ${formatTimestamp(q.createdAt)} |`);
            }
    ...
    // ─── dawinos_get_quote ────────────────────────────────────────────────────────
    server.registerTool('dawinos_get_quote', {
        ...
        try {
            const q = await getDocument(COLLECTIONS.CLIENT_QUOTES, params.quote_id);
            if (!q) {
                return { content: [{ type: 'text', text: `Quote ${params.quote_id} not found.` }] };
            }
            ...
            if (lineItems.length) {
                ...
                for (const [i, item] of lineItems.entries()) {
                    lines.push(`| ${i + 1} | ${String(item['description'] ?? '').substring(0, 40)} | ${item['quantity']} | ${formatCurrency(Number(item['unitPrice']) || 0, currency)} | ${formatCurrency(Number(item['totalPrice']) || 0, currency)} | ${item['taxRateId'] ?? 'no_vat'} |`);
                }
                lines.push('');
            }
            lines.push(`**Subtotal:** ${formatCurrency(Number(q.subtotal ?? q.totalPrice) || 0, currency)}`, `**Tax:** ${formatCurrency(Number(q.tax) || 0, currency)}`, `**Total: ${formatCurrency(Number(q.totalPrice) || 0, currency)}**`, '');
            const qboLines = [];
            if (q.qboSalesOrderId)
                qboLines.push(`Sales Order: ${q.qboSalesOrderDocNumber ?? q.qboSalesOrderId}`);
            if (q.qboInvoiceId)
                qboLines.push(`Invoice: ${q.qboInvoiceDocNumber ?? q.qboInvoiceId}`);
            if (q.qboSyncStatus)
                qboLines.push(`Sync: ${q.qboSyncStatus}`);
            if (q.qboSyncError)
                qboLines.push(`Sync error: ${q.qboSyncError}`);
            if (qboLines.length) {
    ...
    // ─── dawinos_quote_pipeline_analysis ─────────────────────────────────────────
    server.registerTool('dawinos_quote_pipeline_analysis', {
        ...
        try {
            const { items: quotes } = await queryCollection(COLLECTIONS.CLIENT_QUOTES, {
                orderByField: 'createdAt',
                orderByDirection: 'desc',
                limit: params.limit ?? 100,
            });
            if (!quotes.length) {
                return { content: [{ type: 'text', text: 'No quotes to analyse.' }] };
            }
            const currency = quotes[0]?.currency ?? 'UGX';
            const byStatus = {};
            let totalValue = 0;
            ...
            lines.push('', `**QBO Sync:** ${qboSynced}/${quotes.length} quotes synced (${(qboSynced / quotes.length * 100).toFixed(0)}%)`);
            return { content: [{ type: 'text', text: lines.join('\n') }] };
        }
        catch (err) {
            return { isError: true, content: [{ type: 'text', text: `Error: ${String(err)}` }] };
