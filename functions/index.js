...
const { capitalProviderResearch } = require('./src/ai/capitalProviderResearch');
...
exports.capitalProviderResearch = capitalProviderResearch;
...
exports.api = onRequest({
  ...
  timeoutSeconds: 540,
  memory: '1GiB',
  secrets: [ANTHROPIC_API_KEY, GEMINI_API_KEY, QUICKBOOKS_CLIENT_ID, QUICKBOOKS_CLIENT_SECRET]
}, async (req, res) => {
  ...
  try {
    switch (path) {
      ...
      case '/shopify/status':
        ...
        break;
      case '/shopify/orders':
        await getShopifyOrders(req, res);
        break;
      case '/shopify/sync-orders':
        if (req.method === 'POST') {
        ...
        break;
      case '/shopify/update-inventory':
        if (req.method === 'POST') {
          await updateShopifyInventory(req, res);
        } else {
          res.status(405).json({ error: 'Method not allowed' });
        }
        break;
      case '/shopify/locations':
        await getShopifyLocations(req, res);
        break;
      case '/shopify/update-product':
        if (req.method === 'PUT') {
          await updateShopifyProduct(req, res);
        } else {
          res.status(405).json({ error: 'Method not allowed' });
        }
        break;
      ...
      case '/ai/strategy-parse-document':
        ...
        break;
      default:
        res.json({ status: 'ok', message: 'API proxy is running', path, availableEndpoints: ['/customers', '/projects', '/log-activity', '/ai/analyze-brief', '/ai/dfm-check', '/ai/design-chat', '/ai/strategy-research', '/ai/analyze-image', '/ai/feature-cache', '/ai/feature-context', '/notion/sync-milestone', '/ai/strategy-review', '/ai/strategy-parse-document'] });
    }
  } catch (error) {
