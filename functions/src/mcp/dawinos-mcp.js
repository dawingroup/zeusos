...
/**
 * DawinOS MCP Server — Cloud Functions Gen 2 wrapper
 *
 * Thin wrapper around the standalone dawinos-mcp-server package.
 * Uses dynamic import() to avoid bundling @modelcontextprotocol/sdk
 * into the main Cloud Functions bundle.
 *
 * Deploy sequence:
 *   1. cd dawinos-mcp-server && npm run build
 *   2. firebase deploy --only functions:dawinos_mcp
 *
 * Health check:  GET  .../dawinos_mcp
 * MCP endpoint:  POST .../dawinos_mcp  (JSON-RPC 2.0)
 */
const dawinos_mcp = onRequest(
  {
    region: 'us-central1',
    memory: '512MiB',
    timeoutSeconds: 120,
    // Keep CORS locked down for browser clients; MCP clients can still call directly.
    cors: process.env.DAWINOS_MCP_CORS_ORIGINS
      ? process.env.DAWINOS_MCP_CORS_ORIGINS.split(',').map((v) => v.trim()).filter(Boolean)
      : false,
  },
  async (req, res) => {
    const requiredBearer = process.env.DAWINOS_MCP_BEARER_TOKEN;
    if (!requiredBearer) {
    ...
    const setAuthChallenge = (errorCode) =>
      res.set('WWW-Authenticate', `Bearer realm="dawinos-mcp", error="${errorCode}"`);
    ...
    const idToken = parseBearerToken(req.get('x-dawinos-firebase-auth'));
    if (!idToken) {
    ...
    const allowedUids = (process.env.DAWINOS_MCP_ALLOWED_UIDS || '')
      .split(',')
      .map((v) => v.trim())
      .filter(Boolean);
    ...
    req.headers['x-dawinos-actor-id'] = decoded.uid;

    // vendor/dawinos-mcp-server is copied here by the firebase.json predeploy script
    const { createMcpHandler } = await import(
      '../../vendor/dawinos-mcp-server/handler.js'
    );
    return createMcpHandler(req, res);
  },
);

module.exports = { dawinos_mcp };
