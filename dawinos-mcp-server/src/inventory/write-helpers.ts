...
export function emptyEnvelope(mode: 'dry_run' | 'commit'): WriteEnvelope {
  return {
    mode,
    itemsPlanned: 0,
    itemsCommitted: 0,
...
export async function findSkuCollision(
  ...
  for (const doc of snap.docs) {
    if (doc.id !== excludeDocId) return doc.id;
  }
  return null;
}
 ...
 */
export async function findBatchSkuCollisions(
  pairs: Array<{ itemId: string; newSku: string }>
): Promise<WriteWarning[]> {
  const warnings: WriteWarning[] = [];

  // Intra-batch collisions first — cheap, no I/O.
  const newSkuMap = new Map<string, string>(); // newSku → first itemId that claimed it
  for (const p of pairs) {
    if (!p.newSku) continue;
  ...
  const checks = await Promise.all(
    pairs.map(async (p) =>
      p.newSku ? { pair: p, owner: await findSkuCollision(p.newSku, p.itemId) } : null
    )
  );
 ...
 */
export async function scanSkuReferences(
  itemId: string,
  options: { oldSku?: string } = {}
): Promise<WriteWarning[]> {
  const db = getDb();
  const warnings: WriteWarning[] = [];
    ...
    probes.map(async (p) => {
      try {
        ...
        const op: FirebaseFirestore.WhereFilterOp =
          p.field === 'inventoryItemId' ? '==' : 'array-contains';
  ...
  if (options.oldSku) {
    ...
    const skuResults = await Promise.all(
      skuProbes.map(async (p) => {
        try {
          const op: FirebaseFirestore.WhereFilterOp = p.field === 'sku' ? '==' : 'array-contains';
          const snap = await db
...
export async function commitBatch(
  writes: Array<{ ref: DocumentReference; update: Record<string, unknown> }>,
  writer: string
    ...
    try {
      ...
      for (const w of slice) {
        try {
          ...
          failed++;
          errors.push(`${w.ref.id}: ${(e as Error).message}`);
        }
      }
    }
  }
...
export async function loadItem(
  itemId: string
): Promise<{ id: string; data: Record<string, unknown>; ref: DocumentReference } | null> {
  const db: Firestore = getDb();
  const ref = db.collection(COLLECTIONS.INVENTORY_ITEMS).doc(itemId);
  const snap = await ref.get();
  if (!snap.exists) return null;
  return { id: snap.id, data: snap.data() as Record<string, unknown>, ref };
}
 ...
 */
export async function assertItemExists(
  itemId: string
): Promise<{ id: string; data: Record<string, unknown>; ref: DocumentReference }> {
