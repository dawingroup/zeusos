...
import type { WriteEnvelope, ItemDiff, WriteWarning, FieldDiff } from './write-helpers.js';
...
function renderItemDiff(d: ItemDiff): string {
  const header = `  - **${d.sku || '(no sku)'}** — ${d.name || '(no name)'} \`(${d.itemId})\``;
  if (d.changes.length === 0) return `${header}\n    _no field changes_`;
  return [header, ...d.changes.map(renderFieldDiff)].join('\n');
}

function renderWarning(w: WriteWarning): string {
  const prefix =
    w.kind === 'collision'
      ? '❌'
      : w.kind === 'missing'
 ...
 */
export function formatEnvelope(env: WriteEnvelope, toolName?: string): string {
  const DIFF_CAP = 25;
  const lines: string[] = [];
  ...
  if (env.errors.length > 0) {
    ...
    for (const e of env.errors) lines.push(`  - ❌ ${e}`);
    lines.push('');
  }

  if (env.warnings.length > 0) {
    lines.push(`### Warnings (${env.warnings.length})`);
    for (const w of env.warnings) lines.push(renderWarning(w));
    lines.push('');
  }
  ...
  if (env.diffs.length === 0) {
    ...
    for (const d of shown) lines.push(renderItemDiff(d));
