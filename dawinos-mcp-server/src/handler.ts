...
export async function createMcpHandler(req: Request, res: Response): Promise<void> {
    ...
    if (!allowMutations && isMutatingTool(toolName)) {
      ...
      res.status(403).json(
        jsonRpcErrorResponse(
          rpc.id,
          -32000,
          `Tool "${toolName}" is blocked by compliance policy (set DAWINOS_MCP_ALLOW_MUTATIONS=true to override).`
        )
      );
      console.warn(
        `[dawinos-mcp] blocked mutating tool call tool=${toolName} ip=${ip} ua=${req.get('user-agent') ?? 'unknown'}`
      );
      return;
    }
  }
  ...
  console.info(
    `[dawinos-mcp] request requestId=${requestId} actor=${actorId} method=${rpc.method ?? 'unknown'} tool=${toolName || '-'} argsHash=${toolName ? argsDigest : '-'} ip=${ip}`
  );
