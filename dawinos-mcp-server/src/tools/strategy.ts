...
const companyId = (id?: string) => id ?? DEFAULT_ORG_ID;

const paths = {
  docs: (cId: string) => `companies/${cId}/strategyDocuments`,
  doc: (cId: string, docId: string) => `companies/${cId}/strategyDocuments/${docId}`,
  versions: (cId: string, docId: string) => `companies/${cId}/strategyDocuments/${docId}/versions`,
  reviews: (cId: string) => `companies/${cId}/strategy_reviews`,
  review: (cId: string, rId: string) => `companies/${cId}/strategy_reviews/${rId}`,
  sections: (cId: string, rId: string) => `companies/${cId}/strategy_reviews/${rId}/document_sections`,
  section: (cId: string, rId: string, sId: string) =>
    `companies/${cId}/strategy_reviews/${rId}/document_sections/${sId}`,
  auditLog: (cId: string, rId: string) =>
    `companies/${cId}/strategy_reviews/${rId}/section_audit_log`,
};
...
function pillarSummaryLine(p: StrategicPillar): string {
...
export function registerStrategyTools(server: McpServer): void {
  ...
  // ─── dawinos_list_strategy_documents ─────────────────────────────────────────
  server.registerTool('dawinos_list_strategy_documents', {
...
Filter by type, status, scope, or time horizon.`,
    ...
    try {
      ...
      if (params.scope) filters.push({ field: 'scope', op: '==', value: params.scope });
      if (params.time_horizon) filters.push({ field: 'timeHorizon', op: '==', value: params.time_horizon });
      if (params.fiscal_year) filters.push({ field: 'fiscalYear', op: '==', value: params.fiscal_year });

      const { items, total } = await queryCollection<StrategyDocument>(paths.docs(cId), {
        filters,
      ...
      if (!items.length) {
        return { content: [{ type: 'text' as const, text: 'No strategy documents found.' }] };
      }
      ...
      for (const d of items) {
        lines.push(
          `| ${d.id.slice(0, 8)} | ${d.title.slice(0, 40)} | ${d.type} | ${d.status} | ${d.timeHorizon} | ${d.pillars?.length ?? 0} | ${formatTimestamp(d.updatedAt)} |`
        );
      }

      lines.push('', `_Use \`dawinos_get_strategy_document\` with a document ID for full detail._`);
  ...
  // ─── dawinos_get_strategy_document ────────────────────────────────────────────
  server.registerTool('dawinos_get_strategy_document', {
    ...
    const cId = companyId(params.company_id);
    try {
      const doc = await getDocument<StrategyDocument>(paths.docs(cId), params.document_id);
      if (!doc) {
        return { content: [{ type: 'text' as const, text: 'Strategy document not found.' }] };
      }
      ...
      if (pillars.length > 0) {
        ...
        for (const p of sorted) {
          ...
          if (p.owner) lines.push(`Owner: ${p.owner.name}${p.owner.role ? ` (${p.owner.role})` : ''}`);
  ...
  // ─── dawinos_update_strategy_document ─────────────────────────────────────────
  server.registerTool('dawinos_update_strategy_document', {
    title: 'Update Strategy Document',
    description: `Update top-level fields of a strategy document — title, status, description, approval, review schedule, tags. Does NOT modify pillars; use \`dawinos_update_strategy_pillar\` for that.`,
    inputSchema: {
      document_id: z.string().describe('Strategy document ID'),
      company_id: z.string().optional(),
    ...
    try {
      ...
      if (params.fiscal_year !== undefined) updates['fiscalYear'] = params.fiscal_year;
      if (params.quarter !== undefined) updates['quarter'] = params.quarter;
      if (params.approved_by !== undefined) {
        updates['approvedBy'] = params.approved_by;
        updates['approvedAt'] = serverTimestamp();
      }
      if (params.review_frequency !== undefined) updates['reviewFrequency'] = params.review_frequency;
      ...
      return {
        content: [{ type: 'text' as const, text: `Strategy document \`${params.document_id}\` updated.\nFields changed: ${changed.join(', ')}` }],
      };
    } catch (err) {
      return { isError: true, content: [{ type: 'text' as const, text: `Error: ${String(err)}` }] };
    }
  ...
  // ─── dawinos_update_strategy_pillar ───────────────────────────────────────────
  server.registerTool('dawinos_update_strategy_pillar', {
    ...
    const db = getDb();
    try {
      const stratDoc = await getDocument<StrategyDocument>(paths.docs(cId), params.document_id);
      if (!stratDoc) {
        return { isError: true, content: [{ type: 'text' as const, text: 'Strategy document not found.' }] };
      }

      const pillars = stratDoc.pillars ?? [];
      const idx = pillars.findIndex(p => p.id === params.pillar_id);
      if (idx === -1) {
        return { isError: true, content: [{ type: 'text' as const, text: `Pillar ${params.pillar_id} not found in document.` }] };
      }

      const pillar = { ...pillars[idx] };
      const changed: string[] = [];

      if (params.name !== undefined) { pillar.name = params.name; changed.push('name'); }
      if (params.description !== undefined) { pillar.description = params.description; changed.push('description'); }
      if (params.status !== undefined) { pillar.status = params.status; changed.push('status'); }
      if (params.progress !== undefined) { pillar.progress = params.progress; changed.push('progress'); }
      if (params.weight !== undefined) { pillar.weight = params.weight; changed.push('weight'); }
      ...
      return {
        content: [{ type: 'text' as const, text: `Pillar "${pillar.name}" updated.\nFields changed: ${changed.join(', ')}` }],
      };
    } catch (err) {
      return { isError: true, content: [{ type: 'text' as const, text: `Error: ${String(err)}` }] };
    }
  ...
  // ─── dawinos_update_strategy_objective ────────────────────────────────────────
  server.registerTool('dawinos_update_strategy_objective', {
    ...
    try {
      ...
      if (pillarIdx === -1) {
        return { isError: true, content: [{ type: 'text' as const, text: `Pillar ${params.pillar_id} not found.` }] };
      }
      ...
      if (objIdx === -1) {
        return { isError: true, content: [{ type: 'text' as const, text: `Objective ${params.objective_id} not found.` }] };
      }

      const obj = { ...objectives[objIdx] };
      const changed: string[] = [];

      if (params.title !== undefined) { obj.title = params.title; changed.push('title'); }
      if (params.description !== undefined) { obj.description = params.description; changed.push('description'); }
      if (params.status !== undefined) { obj.status = params.status; changed.push('status'); }
      if (params.progress !== undefined) { obj.progress = params.progress; changed.push('progress'); }
      if (params.priority !== undefined) { obj.priority = params.priority; changed.push('priority'); }
      if (params.notes !== undefined) { obj.notes = params.notes; changed.push('notes'); }
      if (params.assignee_name !== undefined) { obj.assigneeName = params.assignee_name; changed.push('assigneeName'); }
      if (params.assignee_id !== undefined) { obj.assigneeId = params.assignee_id; changed.push('assigneeId'); }
      ...
      return {
        content: [{ type: 'text' as const, text: `Objective "${obj.title}" updated.\nFields changed: ${changed.join(', ')}` }],
      };
    } catch (err) {
      return { isError: true, content: [{ type: 'text' as const, text: `Error: ${String(err)}` }] };
    }
  ...
  // ─── dawinos_list_strategy_reviews ────────────────────────────────────────────
  server.registerTool('dawinos_list_strategy_reviews', {
    ...
    try {
      const filters: QueryFilter[] = [];
      if (params.document_id) filters.push({ field: 'documentId', op: '==', value: params.document_id });
      if (params.status) filters.push({ field: 'status', op: '==', value: params.status });

      const { items, total } = await queryCollection<StrategyReview>(paths.reviews(cId), {
        filters,
      ...
      if (!items.length) {
        return { content: [{ type: 'text' as const, text: 'No strategy reviews found.' }] };
      }
      ...
      for (const r of items) {
        lines.push(
          `| ${r.id.slice(0, 8)} | ${(r.title ?? 'Untitled').slice(0, 35)} | ${(r.documentId ?? '—').slice(0, 8)} | ${r.status ?? '—'} | ${formatTimestamp(r.startedAt)} | ${formatTimestamp(r.completedAt)} |`
        );
      }

      lines.push('', `_Use \`dawinos_get_strategy_review\` for full detail, \`dawinos_list_strategy_sections\` to see parsed sections._`);
      return { content: [{ type: 'text' as const, text: lines.join('\n') }] };
    } catch (err) {
      return { isError: true, content: [{ type: 'text' as const, text: `Error: ${String(err)}` }] };
    }
  ...
  // ─── dawinos_get_strategy_review ──────────────────────────────────────────────
  server.registerTool('dawinos_get_strategy_review', {
    ...
    const cId = companyId(params.company_id);
    try {
      const review = await getDocument<StrategyReview>(paths.reviews(cId), params.review_id);
      if (!review) {
        return { content: [{ type: 'text' as const, text: 'Strategy review not found.' }] };
      }

      // Count sections
      const { items: sections } = await queryCollection<DocumentSection>(
        paths.sections(cId, params.review_id),
        { limit: 200 }
      );
      ...
      const lines = [
        ...
        '',
        `_Use \`dawinos_list_strategy_sections\` to see all sections._`,
        `_Use \`dawinos_assess_strategy_section\` and \`dawinos_rewrite_strategy_section\` to improve sections._`,
  ...
  // ─── dawinos_list_strategy_sections ───────────────────────────────────────────
  server.registerTool('dawinos_list_strategy_sections', {
...
Use with \`dawinos_get_strategy_section\` for full content.`,
    inputSchema: {
      review_id: z.string().describe('Strategy review ID'),
      company_id: z.string().optional(),
    ...
    try {
      const filters: QueryFilter[] = [];
      if (params.rewrite_status) filters.push({ field: 'rewriteStatus', op: '==', value: params.rewrite_status });
      if (params.section_type) filters.push({ field: 'sectionType', op: '==', value: params.section_type });

      const { items } = await queryCollection<DocumentSection>(paths.sections(cId, params.review_id), {
        filters,
      ...
      if (!result.length) {
        return { content: [{ type: 'text' as const, text: 'No sections found matching filters.' }] };
      }
      ...
      for (const s of result) {
        const rewriteLabel = s.rewriteStatus && s.rewriteStatus !== 'none'
          ...
          : '—';
        lines.push(
          `| ${s.sectionIndex ?? '?'} | ${'#'.repeat(s.headingLevel ?? 1)} ${s.headingText.slice(0, 45)} | ${s.sectionType ?? '—'} | ${formatScore(s.alignmentScore)} | ${rewriteLabel} |`
        );
      }

      lines.push('', `_Use \`dawinos_get_strategy_section\` with section_id for content and pending rewrites._`);
      return { content: [{ type: 'text' as const, text: truncateIfNeeded(lines.join('\n')) }] };
    } catch (err) {
      return { isError: true, content: [{ type: 'text' as const, text: `Error: ${String(err)}` }] };
    }
  ...
  // ─── dawinos_get_strategy_section ─────────────────────────────────────────────
  server.registerTool('dawinos_get_strategy_section', {
    ...
    const cId = companyId(params.company_id);
    try {
      const section = await getDocument<DocumentSection>(
        paths.sections(cId, params.review_id),
        params.section_id
      );
      ...
      if (section.pendingRewrite) {
        ...
        lines.push('', `_Use \`dawinos_apply_strategy_rewrite\` to approve or reject this rewrite._`);
      }
  ...
  // ─── dawinos_update_strategy_section ──────────────────────────────────────────
  server.registerTool('dawinos_update_strategy_section', {
    title: 'Update Strategy Section Content',
    description: `Manually edit the content of a strategy document section. Use this for direct edits. For AI-suggested rewrites, use \`dawinos_rewrite_strategy_section\` then \`dawinos_apply_strategy_rewrite\`.`,
    inputSchema: {
      review_id: z.string().describe('Strategy review ID'),
      section_id: z.string().describe('Section document ID'),
    ...
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: `Section ${params.section_id} content updated.` }],
      };
    } catch (err) {
      return { isError: true, content: [{ type: 'text' as const, text: `Error: ${String(err)}` }] };
    }
  ...
  // ─── dawinos_apply_strategy_rewrite ───────────────────────────────────────────
  server.registerTool('dawinos_apply_strategy_rewrite', {
...
- **reject**: Clears pendingRewrite, marks rewriteStatus = "rejected"`,
    ...
    try {
      ...
      if (!section) {
        return { isError: true, content: [{ type: 'text' as const, text: 'Section not found.' }] };
      }

      if (!section.pendingRewrite) {
        return { isError: true, content: [{ type: 'text' as const, text: 'No pending rewrite on this section.' }] };
      }
  ...
  // ─── dawinos_get_strategy_audit_log ───────────────────────────────────────────
  server.registerTool('dawinos_get_strategy_audit_log', {
    ...
    try {
      const filters: QueryFilter[] = [];
      if (params.section_id) filters.push({ field: 'sectionId', op: '==', value: params.section_id });
      ...
      }>(paths.auditLog(cId, params.review_id), {
        filters,
      ...
      for (const e of items) {
        const scoreDelta = (e.alignmentScoreBefore != null && e.alignmentScoreAfter != null)
          ...
          : '—';
        lines.push(
          `| ${formatTimestamp(e.timestamp)} | ${e.sectionHeading?.slice(0, 30) ?? '—'} | ${e.changeType} | ${scoreDelta} | ${e.approvedBy ?? e.triggeredBy ?? '—'} | ${(e.rationale ?? '').slice(0, 40)} |`
        );
      }
