...
import type { AdvisoryProject, AllocationGroup } from '../types.js';
...
export function registerAdvisoryTools(server: McpServer): void {
  // ── 1. dawinos_list_project_funds ────────────────────────────────────────────
  server.registerTool(
    'dawinos_list_project_funds',
    {
      title: 'List Advisory Projects (Project Funds)',
      description: `List advisory projects from the DawinOS advisory module with optional filters.
...
programId (exact match), search (partial match on name/projectCode/customerName).`,
    ...
    async (params) => {
      try {
        ...
        const { items, total } = await queryCollection<AdvisoryProject>(collectionPath, {
          filters,
        ...
        if (responseFormat === 'json') {
          const result = {
            ...
            items: filtered.map((p) => serializeDoc(p as unknown as Record<string, unknown>)),
          };
...
|------|------|--------|--------|-------|-----------|-----------|----------------|`;
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error listing advisory projects: ${(err as Error).message}` }],
        };
      }
    },
  );

  // ── 2. dawinos_get_project_fund ──────────────────────────────────────────────
  server.registerTool(
    'dawinos_get_project_fund',
    {
...
- location, tags, programId, projectType, dates`,
    ...
    async (params) => {
      try {
        const project = await getDocument<AdvisoryProject>(ADVISORY_PATHS.projects(), params.project_id);
...
${accountabilitySection}`;
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error fetching advisory project: ${(err as Error).message}` }],
        };
      }
    },
  );

  // ── 3. dawinos_project_expenditures ─────────────────────────────────────────
  server.registerTool(
    'dawinos_project_expenditures',
    {
      title: 'List Project Expenditure Allocations',
      description: `List allocation groups (expenditure allocations) for the DawinOS advisory module.
...
To filter by status: 'Draft' or 'Finalized' (PascalCase).`,
    ...
    async (params) => {
      try {
        ...
        const { items, total } = await queryCollection<AllocationGroup>(collectionPath, {
          filters,
          orderByField: 'date',
          orderByDirection: 'desc',
          limit,
        ...
        const filtered = items.filter((ag) => {
          ...
          const d = ag.date
            ? typeof (ag.date as { toDate?: () => Date }).toDate === 'function'
              ? (ag.date as unknown as { toDate: () => Date }).toDate()
              : new Date(ag.date as unknown as string)
            : null;
          if (!d) return true;
        ...
        if (responseFormat === 'json') {
          const result = {
            ...
            hasMore,
            nextOffset: hasMore ? offset + limit : undefined,
            items: filtered.map((ag) => serializeDoc(ag as unknown as Record<string, unknown>)),
          };
          return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
        }
...
|---------|------|--------|--------|--------|-------------|`;
          ...
          content: [
            {
              ...
              text: truncateIfNeeded(
                `## Expenditure Allocation Groups${params.project_id ? ` — Project ${params.project_id}` : ''}\n\n${header}\n${rows.join('\n')}${pagination}`,
              ),
            },
          ],
        };
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error listing allocation groups: ${(err as Error).message}` }],
        };
      }
    },
  );

  // ── 4. dawinos_accountability_summary ───────────────────────────────────────
  server.registerTool(
    'dawinos_accountability_summary',
    {
...
- projectSummaries: per-project breakdown`,
    ...
    async (params) => {
      try {
        ...
        const overallRate =
          totalDisbursed > 0 ? Math.round((totalAccounted / totalDisbursed) * 100) : null;
        ...
        const healthIndicator =
          overallRate == null ? '⚪' : overallRate >= 90 ? '🟢' : overallRate >= 70 ? '🟡' : '🔴';

        const sortedSummaries = [...projectSummaries].sort(
          (a, b) => (b.unaccountedAmount ?? 0) - (a.unaccountedAmount ?? 0),
        );
...
${summaryRows.join('\n')}`;
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error generating accountability summary: ${(err as Error).message}` }],
        };
      }
    },
  );
