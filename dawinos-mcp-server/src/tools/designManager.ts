...
type DesignProjectRow = { id: string; code?: string; name?: string; customerName?: string };
type DesignItemRow = { id: string; parts?: unknown[]; [k: string]: unknown };
 ...
 */
export function registerDesignManagerTools(server: McpServer): void {
  ...
  server.registerTool('dawinos_search_design_projects', {
    ...
    description:
      'Find `designProjects` by exact `code` (e.g. DF-2026-584) and/or by partial match on `name` / `customerName` (client-side filter, capped).',
    inputSchema: {
      project_code: z
        .string()
        .optional()
      ...
      search: z
        ...
        .optional()
        .describe(
          'Case-insensitive substring on `name` and `customerName` (only if `project_code` is not set, or in addition to narrow results); uses a paginated scan — keep search specific.',
        ),
    ...
    if (params.project_code?.trim()) {
      const { items, total } = await queryCollection<DesignProjectRow>(path, {
    ...
    const { items } = await queryCollection<DesignProjectRow>(path, { limit: 200 });
      ...
      content: [
        {
          type: 'text' as const,
          text: JSON.stringify(
            { count: sliced.length, scanned: items.length, projects: sliced },
            null,
            2,
          ),
        },
  ...
  server.registerTool('dawinos_list_design_items', {
    ...
    const rows = await getSubcollection<DesignItemRow>(
      COLLECTIONS.DESIGN_PROJECTS,
      params.project_id,
      SUB_DESIGN_ITEMS,
    ...
    const summary = rows.map(r => {
      ...
      return { ...serializeDoc(rest as Record<string, unknown>), _partsCount: n };
    });
    return { content: [{ type: 'text' as const, text: JSON.stringify({ count: summary.length, items: summary }, null, 2) }] };
  });
  ...
  server.registerTool('dawinos_get_design_item', {
    ...
    description:
      'Read `designItems/{id}` on a project. Includes `parts` (scene sync rows have `source: "design-studio"`). Large responses may be truncated.',
    inputSchema: {
      project_id: z.string().min(1).describe('Design project document ID'),
    ...
    const row = await getSubDocument<DesignItemRow>(
      COLLECTIONS.DESIGN_PROJECTS,
      params.project_id,
      SUB_DESIGN_ITEMS,
      params.design_item_id,
    );
    if (!row) {
      return {
        isError: true,
        content: [{ type: 'text' as const, text: 'Design item not found.' }],
      };
    }
    let payload: Record<string, unknown> = serializeDoc(row as unknown as Record<string, unknown>) as Record<string, unknown>;
    if (params.parts_only) {
      payload = {
        id: row.id,
        name: (row as { name?: string }).name,
        itemCode: (row as { itemCode?: string }).itemCode,
        parts: row.parts,
