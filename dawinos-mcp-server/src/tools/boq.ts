...
function boqPath(projectId: string, orgId = DEFAULT_ORG): string {
  return `organizations/${orgId}/advisory_projects/${projectId}/boq_items`;
}

/** Build path to a single BOQ item. */
function boqItemPath(projectId: string, itemId: string, orgId = DEFAULT_ORG): string {
...
interface MaterialRate {
  id: string;
  name: string;
  standardRate?: { amount: number; currency: string };
  lastPurchaseRate?: { amount: number; currency: string };
  averageRate?: { amount: number; currency: string };
}
...
export function registerBOQTools(server: McpServer): void {
...
Combine with \`dawinos_apply_formula_to_boq\` to assign formulas, or \`dawinos_boq_cost_summary\` for a cost roll-up.`,
    ...
    try {
      ...
      const { items } = await queryCollection<BOQItem>(path, {
        filters,
      ...
      for (const item of page) {
        ...
        lines.push(
          `| L${lvl} | ${item.hierarchyPath ?? item.itemCode ?? item.itemNumber ?? item.id.slice(0, 8)} | ${item.description.slice(0, 60)} | ${item.unit ?? '-'} | ${qty} | ${fmt(amount)} | ${formulaLabel} | ${matCount > 0 ? matCount : '—'} |`
        );
      }
...
Shows the complete material breakdown per component with required quantity, purchase quantity (MOQ-rounded), unit price, and cost.`,
    ...
    const orgId = params.org_id ?? DEFAULT_ORG;
    try {
      const item = await getDocument<BOQItem>(
        boqPath(params.project_id, orgId),
        params.item_id
      );

      if (!item) {
        return { content: [{ type: 'text' as const, text: 'BOQ item not found.' }] };
      }
      ...
      if (item.materialRequirements && item.materialRequirements.length > 0) {
        ...
        for (const r of reqs) {
          ...
          lines.push(
            `| ${r.materialName} | ${reqQty.toFixed(2)} | **${purQty.toFixed(2)}** | ${moqLabel} | ${r.unit ?? '-'} | ${r.unitPrice ? fmt(r.unitPrice) : '—'} | ${r.totalCost ? fmt(r.totalCost) : '—'} |`
          );
        }

        lines.push('');
        lines.push(`Material cost: **${fmt(materialCost)}**`);
        if (labourCost > 0) lines.push(`Labour cost: **${fmt(labourCost)}**`);
        if (equipCost > 0) lines.push(`Equipment cost: **${fmt(equipCost)}**`);
...
To mark an item as excluded or bulk, set the \`no_formula_required\` or \`is_bulk_item\` flags.`,
    ...
    try {
      ...
      if (params.specification !== undefined) updates['specification'] = params.specification;
      ...
      if (params.rate !== undefined) { updates['rate'] = params.rate; updates['unitRate'] = params.rate; }
      if (params.labor_rate !== undefined) updates['laborRate'] = params.labor_rate;
      if (params.material_rate !== undefined) updates['materialRate'] = params.material_rate;
      if (params.equipment_rate !== undefined) updates['equipmentRate'] = params.equipment_rate;
      if (params.stage !== undefined) updates['stage'] = params.stage;
      if (params.category !== undefined) updates['category'] = params.category;
      if (params.status !== undefined) updates['status'] = params.status;
      if (params.no_formula_required !== undefined) updates['noFormulaRequired'] = params.no_formula_required;
      if (params.is_bulk_item !== undefined) updates['isBulkItem'] = params.is_bulk_item;

      // Recalculate amount if both qty and rate are now known
      if (params.quantity_contract !== undefined || params.rate !== undefined) {
        const existing = await getDocument<BOQItem>(path, params.item_id);
        if (existing) {
...
After running this tool, use \`dawinos_get_boq_item\` to see the computed material breakdown.`,
    ...
    if (!params.formula_id && !params.formula_code) {
      return { isError: true, content: [{ type: 'text' as const, text: 'Provide either formula_id or formula_code.' }] };
    }
    ...
    try {
      // Fetch BOQ item
      const item = await getDocument<BOQItem>(path, params.item_id);
      if (!item) {
        return { isError: true, content: [{ type: 'text' as const, text: 'BOQ item not found.' }] };
      }
      ...
      if (materialIds.length > 0) {
        try {
          const { items: rates } = await queryCollection<MaterialRate>(MATFLOW_PATHS.materials, {
            filters: [],
            limit: 500,
          });
          for (const r of rates) {
            const price = (
              rateSource === 'lastPurchase' ? r.lastPurchaseRate?.amount :
              rateSource === 'average' ? r.averageRate?.amount :
              r.standardRate?.amount
            ) ?? 0;
      ...
      for (const r of requirements) {
        ...
        lines.push(
          `| ${r.materialName} | ${r.requiredQuantity.toFixed(2)} | **${r.purchaseQuantity.toFixed(2)}**${moqNote} | ${r.unit} | ${r.unitPrice ? fmt(r.unitPrice) : '—'} | ${r.totalCost ? fmt(r.totalCost) : '—'} |`
        );
      }

      lines.push('');
      lines.push(`Material cost: **${fmt(totalCost)}**`);
      if (labourCost > 0) lines.push(`Labour (formula default): **${fmt(labourCost)}**`);
      if (equipCost > 0) lines.push(`Equipment (formula default): **${fmt(equipCost)}**`);
...
Use this for a quick project health-check or to identify gaps before a cost estimate.`,
    ...
    try {
      ...
      let excluded = 0;
      let needsFormula = 0;
      const materialMap = new Map<string, { name: string; cost: number }>();
      ...
      for (const item of level4Plus) {
        ...
        const hasMaterials = item.materialRequirements && item.materialRequirements.length > 0;
        if (item.noFormulaRequired) { excluded++; continue; }
        if (item.isBulkItem) { withBulk++; continue; }
        if (item.formulaId && hasMaterials) withFormulas++;
        else if (!item.formulaId && hasMaterials) withCustom++;
        else needsFormula++;
        ...
        for (const r of reqs) {
          ...
          else { materialMap.set(key, { name: r.materialName, cost }); }
        }
      ...
      if (topMaterials.length > 0) {
        ...
        for (const m of topMaterials) {
          if (m.cost > 0) lines.push(`| ${m.name} | ${fmt(m.cost)} |`);
        }
      }
...
Pass \`item_id\` to get requirements for a single BOQ item instead of the whole project.`,
    ...
    try {
      ...
      if (params.item_id) {
        const single = await getDocument<BOQItem>(path, params.item_id);
        if (!single) return { content: [{ type: 'text' as const, text: 'BOQ item not found.' }] };
        boqItems = [single];
      } else {
        const { items } = await queryCollection<BOQItem>(path, { filters: [], limit: 1000 });
        boqItems = items.filter(i =>
          (i.hierarchyLevel ?? 99) >= 4 &&
      ...
      const consolidated = new Map<string, {
        ...
        totalRequired: number; totalPurchase: number;
        moq: number | null; packUnit: string | null;
        unitPrice: number; totalCost: number; itemCount: number;
      }>();
      ...
      for (const r of rows) {
        const moqLabel = r.moq ? `${r.moq}${r.packUnit ? ' ' + r.packUnit : ''}` : '—';
        lines.push(
          `| ${r.name} | ${r.totalRequired.toFixed(2)} | **${r.totalPurchase.toFixed(2)}** | ${moqLabel} | ${r.unit} | ${r.unitPrice ? fmt(r.unitPrice) : '—'} | **${fmt(r.totalCost)}** |`
        );
      }
