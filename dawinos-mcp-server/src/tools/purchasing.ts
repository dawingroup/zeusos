...
export function registerPurchasingTools(server: McpServer): void {
  // ─── Tool 1: List Purchase Orders ────────────────────────────────────────────
  server.registerTool(
    'dawinos_list_purchase_orders',
    {
...
- Recent POs: { "date_from": "2026-01-01", "limit": 10 }`,
    ...
    async (params) => {
      try {
        ...
        if (supplier_id) filters.push({ field: 'supplierId', op: '==', value: supplier_id });
        if (project_id) filters.push({ field: 'projectId', op: '==', value: project_id });

        const { items: pos, total } = await queryCollection<PurchaseOrder>(
        ...
        if (response_format === 'json') {
          const result = {
            ...
            items: paginated.map((po) => serializeDoc(po as unknown as Record<string, unknown>)),
          };
          return {
        ...
        for (const po of paginated) {
          ...
          lines.push(
            `| ${poNum} | ${po.status ?? 'N/A'} | ${po.supplierName ?? 'N/A'} | ${formatCurrency(total_, po.currency)} | ${po.currency ?? 'UGX'} | ${formatTimestamp(po.createdAt)} |`
          );
        }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error listing purchase orders: ${(error as Error).message}` }],
        };
      }
    }
  );
...
The 'total' field contains the grand total (not 'grandTotal').`,
    ...
    async (params) => {
      try {
        ...
        const po = await getDocument<PurchaseOrder>(COLLECTIONS.PURCHASE_ORDERS, id);
        ...
        if (response_format === 'json') {
          return {
            content: [
              { type: 'text', text: JSON.stringify(serializeDoc(po as unknown as Record<string, unknown>), null, 2) },
            ],
          };
        }
        ...
        if (lineItems.length > 0) {
          ...
          for (const item of lineItems) {
            lines.push(
              `| ${item.itemName ?? 'N/A'} | ${item.sku ?? '-'} | ${item.quantity ?? 0} | ${formatCurrency(item.unitPrice, po.currency)} | ${formatCurrency(item.totalPrice, po.currency)} | ${item.receivedQuantity ?? '-'} |`
            );
          }
        } else {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error fetching purchase order: ${(error as Error).message}` }],
        };
      }
    }
  );
...
- Monthly breakdown: { "group_by": "month", "date_from": "2025-01-01" }`,
    ...
    async (params) => {
      try {
        ...
        const { items: pos } = await queryCollection<PurchaseOrder>(COLLECTIONS.PURCHASE_ORDERS, {
          filters,
          orderByField: 'createdAt',
          orderByDirection: 'desc',
          limit: 500,
        ...
        // Aggregate
        const groups = new Map<string, { count: number; totalSpend: number; currency: string }>();
        ...
        for (const po of filtered) {
          ...
          if (group_by === 'supplier') {
            ...
            const ts = po.createdAt;
            const d = ts && typeof (ts as { toDate?: unknown }).toDate === 'function'
        ...
        for (const [key, g] of sortedGroups) {
          ...
          lines.push(
            `| ${key} | ${g.count} | ${formatCurrency(g.totalSpend, g.currency)} | ${formatCurrency(g.totalSpend / g.count, g.currency)} | ${share}% |`
          );
        }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error running spend analysis: ${(error as Error).message}` }],
        };
      }
    }
  );
...
Each line item needs: name, quantity, unit_price, unit, currency (UGX or USD).`,
    ...
    async (params) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error creating purchase order: ${(err as Error).message}` }],
        };
      }
    }
  );
...
rejection_reason is required when status=rejected.`,
    ...
    async (params) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error updating PO status: ${(err as Error).message}` }],
        };
      }
    }
  );
...
Best used for POs in draft or approved status.`,
    ...
    async (params) => {
      try {
        ...
        if (params.notes !== undefined) { update['notes'] = params.notes; changed.push('notes'); }
        if (params.supplier_name !== undefined) { update['supplierName'] = params.supplier_name; changed.push('supplierName'); }
        if (params.supplier_id !== undefined) { update['supplierId'] = params.supplier_id; changed.push('supplierId'); }
        if (params.project_id !== undefined) { update['linkedProjectId'] = params.project_id || null; changed.push('linkedProjectId'); }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error updating purchase order: ${(err as Error).message}` }],
        };
      }
    }
  );
