...
import type { ManufacturingOrder, BOMEntry, MaterialConsumption, StageTransition } from '../types.js';
...
export function registerManufacturingTools(server: McpServer): void {
  // ─── Tool 1: List Manufacturing Orders ────────────────────────────────────────
  server.registerTool(
    'dawinos_list_manufacturing_orders',
    {
...
Stage field on documents is 'currentStage' (not 'productionStage').`,
    ...
    async (params) => {
      try {
        const { status, current_stage, priority, project_id, limit, offset, response_format } =
          params;
        ...
        if (current_stage) filters.push({ field: 'currentStage', op: '==', value: current_stage });
        if (priority) filters.push({ field: 'priority', op: '==', value: priority });
        if (project_id) filters.push({ field: 'projectId', op: '==', value: project_id });

        const { items: mos, total } = await queryCollection<ManufacturingOrder>(
          COLLECTIONS.MANUFACTURING_ORDERS,
          {
            filters,
        ...
        if (response_format === 'json') {
          const result = {
            ...
            items: mos.map((mo) => serializeDoc(mo as unknown as Record<string, unknown>)),
          };
        ...
        for (const mo of mos) {
          ...
          lines.push(
            `| ${moNum} | ${mo.name ?? 'N/A'} | ${mo.status ?? 'N/A'} | ${mo.currentStage ?? 'N/A'} | ${emoji} ${mo.priority ?? 'N/A'} | ${formatTimestamp(mo.targetCompletionDate)} | ${formatCurrency(mo.totalEstimatedCost)} |`
          );
        }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error listing manufacturing orders: ${(error as Error).message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'dawinos_get_manufacturing_order',
    {
...
(not 'bom', 'materials', or 'steps').`,
    ...
    async (params) => {
      try {
        ...
        const mo = await getDocument<ManufacturingOrder>(COLLECTIONS.MANUFACTURING_ORDERS, id);
        ...
        const [bomEntries, materialConsumptions, stageTransitions] = await Promise.all([
          getSubcollection<BOMEntry>(
            COLLECTIONS.MANUFACTURING_ORDERS,
            id,
            MO_SUBCOLLECTIONS.BOM_ENTRIES,
            { limit: 200 }
          ),
          getSubcollection<MaterialConsumption>(
            COLLECTIONS.MANUFACTURING_ORDERS,
            id,
            MO_SUBCOLLECTIONS.MATERIAL_CONSUMPTIONS,
            { orderByField: 'consumedAt', orderByDirection: 'desc', limit: 50 }
          ),
          getSubcollection<StageTransition>(
            COLLECTIONS.MANUFACTURING_ORDERS,
            id,
            MO_SUBCOLLECTIONS.STAGE_TRANSITIONS,
            { orderByField: 'transitionedAt', orderByDirection: 'desc', limit: 50 }
          ),
        ]);
        ...
        if (response_format === 'json') {
          return {
            content: [
              {
                type: 'text',
                text: JSON.stringify(serializeDoc(mo as unknown as Record<string, unknown>), null, 2),
              },
            ],
          };
        }
        ...
        if (bomEntries.length > 0) {
          ...
          for (const b of bomEntries) {
            lines.push(
              `| ${b.itemName ?? b.name ?? 'N/A'} | ${b.sku ?? '-'} | ${b.quantity ?? '-'} | ${formatCurrency(b.unitCost)} | ${formatCurrency(b.totalCost)} | ${b.status ?? '-'} |`
            );
          }
        }
        ...
        if (materialConsumptions.length > 0) {
          ...
          for (const c of materialConsumptions) {
            lines.push(
              `| ${c.itemName ?? 'N/A'} | ${c.quantity ?? '-'} | ${formatCurrency(c.costPerUnit)} | ${formatCurrency(c.totalCost)} | ${formatTimestamp(c.consumedAt)} |`
            );
          }
        }
        ...
        if (stageTransitions.length > 0) {
          ...
          for (const t of stageTransitions) {
            lines.push(
              `| ${t.fromStage ?? '-'} | ${t.toStage ?? '-'} | ${t.transitionedByName ?? t.transitionedBy ?? '-'} | ${formatTimestamp(t.transitionedAt)} | ${t.notes ?? '-'} |`
            );
          }
        }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error fetching manufacturing order: ${(error as Error).message}` }],
        };
      }
    }
  );
...
Fetches up to 500 non-cancelled MOs for aggregation.`,
    ...
    async (params) => {
      try {
        ...
        const { items: mos } = await queryCollection<ManufacturingOrder>(
          COLLECTIONS.MANUFACTURING_ORDERS,
          {
            filters: [],
            orderByField: 'createdAt',
        ...
        for (const mo of mos) {
          ...
          if (!['completed', 'cancelled'].includes(st) && mo.targetCompletionDate) {
            const dueMs = typeof (mo.targetCompletionDate as { toDate?: unknown }).toDate === 'function'
              ? (mo.targetCompletionDate as { toDate: () => Date }).toDate().getTime()
              : 0;
            if (dueMs > 0 && dueMs < now) overdueCount++;
          }
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error generating production summary: ${(error as Error).message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'dawinos_create_manufacturing_order',
    {
...
Optional: bom (bill of materials), project_id, instructions.`,
    ...
    async (params) => {
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error creating manufacturing order: ${(err as Error).message}` }],
        };
      }
    }
  );
...
Actions and their effects:
  ...
  cancel        → status: cancelled (from any non-terminal status)`,
    ...
    async (params) => {
      try {
        ...
        const moNumber = (data['moNumber'] as string) ?? params.mo_id;

        const STAGE_ORDER = ['queued', 'cutting', 'assembly', 'finishing', 'qc', 'ready'] as const;
        ...
        if (statusChange) historyEntry['statusChange'] = statusChange;
        ...
        const newStage = (update['currentStage'] as string) ?? currentStage;
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error advancing MO stage: ${(err as Error).message}` }],
        };
      }
    }
  );
  ...
  server.registerTool(
    'dawinos_update_manufacturing_order',
    {
...
Status and stage changes must use dawinos_advance_mo_stage.`,
    ...
    async (params) => {
      try {
        ...
        if (params.instructions !== undefined) { update['instructions'] = params.instructions; changed.push('instructions'); }
        if (params.notes !== undefined) { update['handoverNotes'] = params.notes; changed.push('handoverNotes'); }
        if (params.priority !== undefined) { update['priority'] = params.priority; changed.push('priority'); }
        ...
        await ref.update(update);
        const moNumber = (snap.data()?.['moNumber'] as string) ?? params.mo_id;
        ...
        return {
          isError: true,
          content: [{ type: 'text', text: `Error updating manufacturing order: ${(err as Error).message}` }],
        };
      }
    }
  );
